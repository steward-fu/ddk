...
--*/
{
   ...
   //
   for (c = 0; c < TbAtm155MaxRegistryEntry; c++)
   {
...
#ifndef NEW_DMA
               //
               //  Let's to remember the number of allocated map registers
               //  to release the map registers bufffer later.
               //
               MapRegisters = pSar->XmitDmaQ.TotalMapRegisters;
...
#ifndef NEW_DMA
               if (NULL != pAdapter->pMapRegisters)
               {
                   NdisFreeSpinLock(&pSar->XmitDmaQ.FreeMapRegisterQ.lock);

                   //
                   //  free up the buffer for managing map registers.
                   //
                   FREE_MEMORY(
                       pAdapter->pMapRegisters,
                       (sizeof(MAP_REGISTER) * MapRegisters));
               }
#endif
...
#ifndef NEW_DMA
               //
               //	Free the map registers used by the adapter.
               //
               NdisMFreeMapRegisters(pAdapter->MiniportAdapterHandle);
#endif
...
--*/
{
   ...
   TB155PCISAR_CNTRL2      regControl2;
   TX_REPORT_BASE          regTxReportBase;
   RX_REPORT_BASE          regRxReportBase;
   TX_FS_LIST_PTRS         regTxFsListPtrs;
   TX_ABR_NRM_TRM          regCTxAbrNrmTrm;
   TX_ABR_ADTF             regTxAbrADTF;
...
#ifndef NEW_DMA
   pSar->XmitDmaQ.FreeMapRegisters = pSar->XmitDmaQ.TotalMapRegisters;
#endif
   // 
...
--*/
{
   ...
   UINT                c;
   UINT                MapRegisters;
   UINT                MaxMapRegisters;
   PMAP_REGISTER       pMapRegisterBuffers;
   ...
   do
   {
...
#ifndef NEW_DMA
       //
       //	If the number of map registers has an override in the registry
       //	the attempt to allocate that many.  Otherwise allocate what we can
       //	get (within reason).
       //
       if (pAdapter->RegistryParameters[TbAtm155NumberOfMapRegisters].fPresent)
       {
           MapRegisters = pAdapter->RegistryParameters[TbAtm155NumberOfMapRegisters].Value;
       }
       else
       {
           MapRegisters = MAXIMUM_MAP_REGISTERS;
       }

       if (MapRegisters > MAXIMUM_MAP_REGISTERS)
       {
           MapRegisters = MAXIMUM_MAP_REGISTERS;
       }

       //
       //	Allocate as many as the system lets us.
       //
       while (MapRegisters > (MINIMUM_MAP_REGISTERS - 1))
       {
           //
           //	Map registers are only used on transmit so we only save
           //	them with the transmit DMA queue.
           //
       ...
       {
           DBGPRINT(DBG_COMP_INIT, DBG_LEVEL_ERR,
               ("Unable to allocate minimum map registers!\n"));

           break;
       }

       DBGPRINT(DBG_COMP_INIT, DBG_LEVEL_INFO,
           ("Allocated %u map registers to use\n", MapRegisters));

       pSar->XmitDmaQ.TotalMapRegisters = MapRegisters;

       DBGPRINT(DBG_COMP_INIT, DBG_LEVEL_INFO,
           ("Using %u map registers for the adapter\n", MapRegisters));

       //
       //	Allocate memory for buffer of map registers.
       //
       ALLOCATE_MEMORY(
               &Status, 
               &pMapRegisterBuffers,
       ...
       {
           DBGPRINT(DBG_COMP_INIT, DBG_LEVEL_ERR,
               ("Unable to allocate information buffer for the Map Registers\n"));

           break;
       }
	
       ZERO_MEMORY(pMapRegisterBuffers, (sizeof(MAP_REGISTER) * MapRegisters));

       // Save the starting address of map register's buffer.
       pAdapter->pMapRegisters = pMapRegisterBuffers;
       pXmitDmaQ = &pSar->XmitDmaQ;

       InitializeMapRegisterQueue(&pXmitDmaQ->FreeMapRegisterQ);

       NdisAcquireSpinLock(&pXmitDmaQ->FreeMapRegisterQ.lock);

       for (c = 0; c < MapRegisters; c++, pMapRegisterBuffers++)
       {
           // 
           //  Set an index to each structure and link to the free list.
           //
           pMapRegisterBuffers->MapRegisterIndex = (USHORT)c;
       ...
       NdisReleaseSpinLock(&pXmitDmaQ->FreeMapRegisterQ.lock);
   ...
   if (NDIS_STATUS_SUCCESS != Status)
   {
       ...
       if (NULL != pAdapter->pMapRegisters)
       {
           NdisFreeSpinLock(&pXmitDmaQ->FreeMapRegisterQ.lock);
...
#ifndef NEW_DMA
           if (0 != pSar->XmitDmaQ.TotalMapRegisters)
           {
               NdisMFreeMapRegisters(pAdapter->MiniportAdapterHandle);
           }
#endif // NEW_DMA
