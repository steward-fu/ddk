...
--*/
{
   ...
   {
       case OID_TBATM_WRITE_PEEPHOLE:

           //
           //  Vendor Specific OID, Peep-hole write
           //
           DataLength = sizeof(TB_PEEPHOLE);
           if (pRequest->DATA.SET_INFORMATION.InformationBufferLength < DataLength)
           {
               pRequest->DATA.SET_INFORMATION.BytesNeeded = DataLength;
               Status = NDIS_STATUS_INVALID_LENGTH;
               break;
           }

           pPeepholePara = pRequest->DATA.SET_INFORMATION.InformationBuffer;
           if ((pPeepholePara->access_device & 0x01) == 0x01) 
           {
               //
               // Peripheral, access devices via peep-hole
           ...
           {
               pPeepholePara->status = 1;
               pRequest->DATA.SET_INFORMATION.BytesRead = DataLength;
           }
           else
           {
               pPeepholePara->status = 0;
       ...
       case OID_TBATM_WRITE_CSR:

           //
           // Vendor Specific OID,    CSR write
           //
           DataLength = sizeof(TB_CSR);
           if (pRequest->DATA.SET_INFORMATION.InformationBufferLength < DataLength)
           {
           ...
           pCsrPara = pRequest->DATA.SET_INFORMATION.InformationBuffer;
           ...
           if (pCsrPara->address <= 0x36)
           {
               ...
               //
               CsrVirtualAddr = (PUCHAR)pHwInfo->TbAtm155_SAR + (pCsrPara->address << 2);
               TBATM155_WRITE_PORT(CsrVirtualAddr, pCsrPara->data);
