/*++
    ...
    Author:
        Michael Tsang (MikeTs) 6-May-2002
...
SendSyncIoctl(
    ...
    IN  ULONG             dwcbInputBuffer,
    OUT PVOID             OutputBuffer OPTIONAL,
    IN  ULONG             dwcbOutputBuffer
    );
    ...
    )
{
    ...
    PAGED_CODE();
    TRACEENTER(("(devext=%p,IoControlCode=%x,InputBuffer=%p,InLen=%d,OutputBuffer=%p,OutLen=%d)\n",
    ...
    )
{
    ...
    PAGED_CODE();
    TRACEENTER(("(devext=%p,irp=%p,fEnable=%x)\n", devext, irp, fEnable));
    ...
    else
    {
        LogError(ERRLOG_OPEN_KBDPORT_FAILED,
                 ...
                 NULL,
                 NULL);
        TRACEERROR(("Failed to send Create IRP to keyboard port driver (status=%x).\n",
                    status));
    }
    ...
    )
{
    ...
    TRACEENTER(("(devobj=%p,InputStart=%p,InputEnd=%p,pdwcConsumed=%p,cConsumed=%d)\n",
    ...
    while (dwcDataInQueue > 0)
    {
        if (!IsListEmpty(&devext->PendingIrpList))
        {
            ...
            else
            {
                ...
                //
                InsertHeadList(&devext->PendingIrpList,
                               &PendingIrp->Tail.Overlay.ListEntry);
            }
            InputDataStart++;
            dwcDataInQueue--;
            (*pdwcConsumed)++;
        ...
        {
            TRACEWARN(("No pending ReadReport irp.\n"));
            break;
        }
    }
    ...
    )
{
    ...
    TRACEENTER(("(devext=%p,MakeCode=%x,Flags=%x,Report=%p)\n",
...
/*0x58*/0x45
    ...
    TRACEINFO(3, ("MakeCode=%x,Flags=%x\n", wMakeCode, wFlags));
    if (wFlags & KEY_E1)
    {
        TRACEASSERT(wMakeCode == 0x1d);
    ...
    else if (wFlags & KEY_E0)
    {
        ...
        else
        {
            ...
            {
                devext->dwfHBtn &= ~HBTNF_KBD_E0;
                TRACEWARN(("Failed to find %x in E0CodeTable!\n",
                           wMakeCode));
            }
        }
    }
    ...
    else
    {
        ...
        if (!fSendReport)
        {
            TRACEWARN(("Failed to find %x in MakeCodeTable!\n",
                       wMakeCode));
        }
    }
    ...
    if (fSendReport)
    {
        ...
        else
        {
            ...
            if (wFlags & KEY_BREAK)
            {
                ...
                if (!fSendReport)
                {
                    TRACEWARN(("Failed to find make slot for %x!\n", bUsage));
                }
            }
            else
            {
                ...
                {
                    TRACEWARN(("Failed to find empty slot for %x!\n", bUsage));
                }
            }
        }
    ...
    )
{
        ...
        sizeof(HID_XFER_PACKET))
    {
        ...
        LogError(ERRLOG_INVALID_BUFFER_SIZE,
                 ...
                 NULL,
                 NULL);
        TRACEWARN(("invalid transfer packet size (len=%d).\n",
    ...
    else if (XferPacket->reportBufferLen != sizeof(HID_OUTPUT_REPORT))
    {
        ...
        LogError(ERRLOG_INVALID_BUFFER_SIZE,
                 ...
                 NULL,
                 NULL);
        TRACEWARN(("invalid output report size (len=%d).\n",
    ...
    else if (XferPacket->reportId != REPORTID_LED)
    {
        ...
        LogError(ERRLOG_INVALID_PARAMETER,
                 ...
                 NULL,
                 NULL);
        TRACEWARN(("invalid output report ID (ReportID=%d).\n",
                   XferPacket->reportId));
    }
    else
    {
