    ...
    ULONG           DevNode) 
{
    ...
    pDev = InitializeHardwareNode(DevTable, DevNode);
    if (! pDev) {
        return (NULL);
    }
    ...
    pDev->pRegisters = _MapPhysToLinear(
                           pDev->CMConfig.dMemBase[0], 
                           pDev->CMConfig.dMemLength[0], 0);
    ...
    DISPDBG(("Mapped P3 Registers. Phys  :0x%x", pDev->CMConfig.dMemBase[0])); 
    ...
    ULONG           DevNode)
{
    ...
    pDev = InitializeDeviceNode(DevNode);

    pDevWithVGAEnabled = pDev;

    DISPDBG(("InitializeDynamicInit"));

    InitializePrimaryDispatchTable();

    DISPDBG(("Install Power Management callback"));
    _SHELL_HookSystemBroadcast(
        (SYSBHOOK_CALLBACK)&ShellBroadcastCallback, (DWORD)DevTable, 0);
    ...
    BOOL            Destructive)
{
    ...
    DISPDBG(("InitializeHardware"));

    //
    // Read the location of the general configuraion 
    // information from the registry.
    //

    ulValue = SIZE_INFORMATION_KEY-18;
    strcpy(pDev->szInformationKey, REGKEYROOT);
    if (CM_Read_Registry_Value( 
            pDev->DevNode, 
            szDefaultKey, 
    ...
    if (! InitializeChip(pDev)) {

        DISPDBG(("InitializeChip Failed"));
        return (NULL);
    }
    ...
    if (! InitializeDMA(pDev)) {

        DISPDBG(("InitializeDMA Failed"));
        return (NULL);
    }

    if (_RegOpenKey(
            HKEY_LOCAL_MACHINE, 
            pDev->szInformationKey, &hKey) != ERROR_SUCCESS) {
    ...
    DISPDBG(( "Initialized Sucessfully. pGlintDev = 0x%x", pDev));

    return (pDev);
}

void 
InitializeCachedMemoryTimings(
    PDEVICETABLE    pDev)
{
    ULONG           ulValue, ulSize;

    //
    // On P3 this has to be hard coded. Initially assume 4 banks.
    //
...
DWORD 
InitializeChipAndMemoryPreSizing(
    PDEVICETABLE    pDev)
{
    DWORD           ulValue;
    ...
    if (pDev->pDeltaDev->ChipID == DELTA_ID) {
        ...
        //
        // Probably a Permedia reference board with Delta.
        //

        pDev->wBoardID = BOARD_PERMEDIA_DELTA;  
    } else {

        //
        // Probably a Permedia reference board.
        //

        pDev->wBoardID = BOARD_PERMEDIA;        
    }

    //
    // Load initial values into critical registers
    //

    ProcessInitializationTable(pDev);
    ...
    LOAD_GLINT_CTRL_REG(ApertureTwo,        0x0);   // (!!!) P2 & P3 difference

    //
    // Setup Bypass write mask
    //

    LOAD_GLINT_CTRL_REG(BypassWriteMask,    0xFFFFFFFF);
    ...
    if (pDev->dwFlags & GMVF_DELTA) {
        ...
        LOAD_GLINT_CTRL_REG(
            DeltaDisconnectControl, DISCONNECT_INPUT_FIFO_ENABLE);
    } else {
    ...
    PDEVICETABLE    pDev)
{
    ...
    SYNC_WITH_GLINT;
    InitializeChipAndMemoryPreSizing(pDev);

    //
    // FBWindowBase can be destroyed on a chip reset. 
    // Fix up our cache for it here
    //
    ...
    BOOL            Destructive)
{
    ...
    if (pDev->dwFlags & GMVF_DELTA) {
        ...
        if ((dwRegBase & (1 << 17)) == 
                (pDeltaDev->CMConfig.dMemBase[0] & (1<<17))) {
            ...
            //
            // We have to remap the delta to be inside the delta region of 
            // the glint address space. The delta region is PCI region 4 on
            // Permedia, 3 on TX and SX
            ...
            if (pDev->CMConfig.dMemLength[4] == 256*1024) {
                ...
                if (pDev->CMConfig.dMemLength[3] == 256*1024) {

                    deltaRegion = 3;
                } else {

                    DISPDBG((
                        "Delta bit 17 bug: Neither PCI region 3 or 4 is 256K in size"));
                    return (NULL);
                }
            }
            ...
            DISPDBG((
                "Delta Base0:0x%x Glint Delta Region:0x%x Mapping at 0x%x",
    ...
    DISPDBG((
        "Mapped Control Registers. Phys:0x%x Virt:0x%x Length:0x%x", 
        dwRegBase, pR, pDev->CMConfig.dMemLength[0])); 

    InitializeCachedMemoryTimings(pDev);
    ...
    //
    // NOTE : (!!) Significant changes are made in this function
    //
    ...
    //
    // The 2nd aperture points is the FRAMEBUFFER 
    //

    nFBRegion = 2;

    sFB = pDev->CMConfig.dMemLength[nFBRegion];
    pFB = _MapPhysToLinear( 
              pDev->CMConfig.dMemBase[nFBRegion],
              pDev->CMConfig.dMemLength[nFBRegion], 
              0);         
    ...
    //
    // Record the fram buffer location
    //
    ...
    if (Destructive) {
        ...
        //
        // Save the 1st and 2st DWORD, since they are actually used by
        // the boot screen. 
        //

        DWORD   dwVGAPix1, dwVGAPix2;

        dwVGAPix1 = pFB[0];
        dwVGAPix2 = pFB[1];

        //
        // Figuring out the actual size of Frame Buffer
        //
        // Important fact :
        //
        //    P3 is completely different in how its memory behave from P2,
        //    the part of the aperture which doesn't have real memory is 
        //    aliased to the beginning of the aperture which has physical 
        //    memory.
        //
        // A little trick is need to handle this situation
        //

        sFB >>= 2;

        for (pV = pFB + sFB - FB_PROBE_SIZE; pV >= pFB; pV -= FB_PROBE_SIZE) {

            *pV = (DWORD)pV; // Write the address itself at the address
        }

        //
        // Make sure the magic numbers are indeed written into the video memory
        //
        ...
        for (pV = pFB + sFB - FB_PROBE_SIZE; pV >= pFB; pV -= FB_PROBE_SIZE) {
            ...
            if ((*pV) == (DWORD)pV) {
                ...
                DISPDBG(("Correct addr found at 0x%x words", (DWORD)pV));
                break;
            }
        }

        //
        // Calculate the true of the frame buffer
        //
    ...
    if (_RegQueryValueEx(
        ...
        //
        // Key doesn't exist - default WC on
        //
    ...
    if (writeCombine && 
        TestWCCapability() &&
        SetVariableMTRRToWC(pDev->CMConfig.dMemBase[nFBRegion], sFB)) {

        pDev->dwFlags |= GMVF_FRAME_BUFFER_IS_WC;
    }

    DISPDBG((
        "Memory Sizes Detected: FB:0x%x, LB:0x%x LB Depth is %d bits", 
        sFB, sLB, dLB));
    ...
    PDEVICETABLE    pDev)
{
    ...
    DISPDBG(("InitializeSharedData for 0x%x", pDev));
    DISPCHIP((pDev));
    ...
    if (pDev->dwFlags & GMVF_DELTA) {
        ...
        pGLInfo->dwDisconCtrl = 
            (unsigned long)OFFSET_GLINT_REG(DeltaDisconnectControl) + 
            GLINT_REGISTERS_OFFSET;
        pGLInfo->pIntEnable   = 
            (unsigned long)&(pDev->pRegisters->Glint.DeltaIntEnable);
        pGLInfo->pIntFlags    = 
            (unsigned long)&(pDev->pRegisters->Glint.DeltaIntFlags);

    } else {

        pGLInfo->dwDisconCtrl = 
            (unsigned long)OFFSET_GLINT_REG(FIFODiscon) + 
            GLINT_REGISTERS_OFFSET;
        pGLInfo->pIntEnable   = 
            (unsigned long)&(pDev->pRegisters->Glint.IntEnable);
        pGLInfo->pIntFlags    = 
            (unsigned long)&(pDev->pRegisters->Glint.IntFlags);
    }
    ...
    //
    // Update the base of the glint configuration key. 
    // Shouldbe pDev->szInformationKey - "\Display"
    // 
    ...
    pGLInfo->dwRegSel = AllocateSelector(
                            pDev->pRegisters, 
                            pDev->CMConfig.dMemLength[0] >> 12);
    pGLInfo->dwFBSel  = AllocateSelector(
                            pDev->pFrameBuffer, 
                            pDev->dwFrameBufferSize >> 12);

    //
    // P3 doesn't have a LocalBuffer, better to give back a null selector
    //

    if ((pDev->pLocalBuffer) && (pDev->dwLocalBufferSize)) {
        
        pGLInfo->dwLBSel  = AllocateSelector(
                                pDev->pLocalBuffer, 
                                pDev->dwLocalBufferSize >> 12);
    } else {
    ...
    //
    // Decide whether the video memory is SDRAM
    //

    READ_GLINT_CTRL_REG(LocalMemCaps, i);
    if (i & (1 << 28)) {
        pGLInfo->bDRAMBoard = TRUE;
    } else {
        pGLInfo->bDRAMBoard = FALSE;
    }

    DISPDBG(("InitializeSharedData Done"));
    ...
    PDEVICETABLE    pDev)
{
    ...
    if (pDev->IRQHandle != 0) {
        ...
        DISPDBG(("InitializeInterupts called when already enabled. Redundant call"));
        return (1);
    }
    ...
    PDEVICETABLE    pDev) 
{
    ...
    if (DMABuffers.dwFontCacheSize != 0) {
        ...
        if (! Result) {
            ...
            DISPDBG((
                "Font Cache allocated: V:0x%x nPages:0x%x Segment:0x%x",
                Virt, 
    ...
    if ((pDev->ChipID == PERMEDIA2_ID) || 
        (pDev->ChipID == TIPERMEDIA2_ID) ||
        (pDev->ChipID == PERMEDIA3_ID)) {
        ...
        if (! Result) {
            ...
            DISPDBG((
                "2D DMA buffer allocated: V:0x%x P:0x%x nPages:0x%x",
    ...
    PDEVICETABLE    pDev)
{
    ...
    // If we have either of the supported block fill sizes, 
    // set them in FBModeSel
    ...
    PDEVICETABLE    pDev)
{
    ...
    DISPDBG(("InitializeDisplay"));
    DISPCHIP((pDev));

    // Initialize all the registers that matter 
    InitializeChip(pDev);
    ...
    // Note: some of the device driver routines use optimizations which 
    // require the partial products for half and quarter of the current
    // resolution.  They are calculated here at startup instead of each
    // time they are required.
    // NOTE ALSO this assumes partial products exist for half and quarter 
    // values. this is not the case for example for 800x600 mode.
    //

    ulValue = FindPartialProducts(
                  pDev, 
                  pDev->pGLInfo->DesktopDisplayInfo.ddiXRes/2);
    pDev->pGLInfo->dwPartialProducts2 = ulValue;

    ulValue = FindPartialProducts(
                  pDev, 
                  pDev->pGLInfo->DesktopDisplayInfo.ddiXRes/4);
    pDev->pGLInfo->dwPartialProducts4 = ulValue;

    ulValue = FindPartialProducts(
                  pDev, pDev->pGLInfo->DesktopDisplayInfo.ddiXRes);
    ...
    //
    // Set up the video memory heap related information
    //

    pDev->pGLInfo->dwOffscreenBase = pDev->pGLInfo->dwScreenBase +
                                        pDev->pGLInfo->dwScreenWidthBytes *
                                            pDev->pGLInfo->dwScreenHeight;
...
InitializeMode(
    PDEVICETABLE    pDev)
{
    DISPDBG(("InitializeMode"));
    DISPCHIP((pDev));

    if (! pDev->IRQHandle) {

        // Initialize the interrupts
        if (! InitializeInterrupts(pDev)) {

            DISPDBG(("InitializeInterrupts Failed"));
            return (NULL);
        }
    } 

    //
    // Get the clock speed from ROM
    //
    ...
    //
    // which would be the value we get from the ROM
    //

    if (pDev->ChipClockSpeed == 0) {
        // PERMEDIA3_DEFAULT_CLOCK_SPEED doesn't looks like a good default
        pDev->ChipClockSpeed = 100 * 1000 * 1000; 
    }

    //
    // Set the reference clock speed
    //

    pDev->RefClockSpeed = 14318200;

    if (! InitializeVideo(pDev)) {

        DISPDBG(("InitializeVideo Failed"));
        return (0);
    }
   
    if (! InitializeDisplay(pDev)) {

        DISPDBG(("InitializeDisplay Failed"));
        return (0);
    }
...
//
// Procedure to completly reinitialise the display after it has been 
...
//
// This function needs to translate the current driver data
    ...
    DWORD           VirtualMachine)
{
...
#else
    Result = (DWORD)_PageCommit(
                        Base, 
                        nPages, 
                        PD_FIXEDZERO, 
                        0, 
                        (PC_FIXED + PC_USER + PC_WRITEABLE));
#endif //#ifndef WIN98
    ...
    pDev->DataSegment = AllocateSelector(
                            (DWORD *)NewDSBase, FINAL_DATA_SEGMENT_SIZE>>12);
    ...
    //
    // dwDMABufferPhys is a temporary list of the locations 
    // to store the new segment
    //
    ...
    DEVNODE         DevNode)
{
    ...
    //
    // Wish for the trivial case where the device ID is in the list of ID's 
    // we know about.
    //
    ...
    for (i = 0; i < MAX_GLINT_DEVICES; i++) {
        ...
        if ((DevTable[i].DevNode == DevNode) || 
            (DevTable[i].ActualDevNode == DevNode)) {
    ...
    if (cRet != CR_SUCCESS) {
        ...
        DISPDBG((
            "FindpDevFromDevNode: Getting ID of Devnode failed - Error %d", 
            cRet));
    ...
    // at IDing the device we require, search for a non delta device on the
    // same Bus and Device ID as the current node
    //

    GetBusDevFnNumbers(DevNodeIDBuffer, &Bus, &Dev, &Fn);

    DISPDBG((
        "FindpDevFromDevNode: %s Bs%x Dv%x Fn%x", 
        DevNodeIDBuffer, Bus, Dev, Fn));
    ...
    for (i = 0; i < MAX_GLINT_DEVICES; i++) {
        ...
        if ((DevTable[i].ChipID != DELTA_ID) &&
            ...
            DISPDBG((
                "FindpDevFromDevNode: Found 3dlabs device on same bus and function"));
            DISPCHIP((&(DevTable[i])));
    ...
    for (i = 0; i < MAX_GLINT_DEVICES; i++) {
        ...
        if ((DevTable[i].ChipID != DELTA_ID) && (DevTable[i].pRegisters == 0)) {
            ...
            // 
            // Point the DevNode to the correct place to get registry 
            // information.
            //

            DISPDBG((
                "FindpDevFromDevNode: Found first uninitialised 3dlabs device"));
...
FindDevNodeForCurrentlyActiveVGA()
{
    ...
    DISPDBG(("FindDevNodeForCurrentlyActiveVGA() is called.\n"));
    ...
    CLIENT_STRUCT  *Regs)
{
    ...
    switch (Regs->CRS.Client_ECX) {
        ...
        case MINIVDD_GETGLINFO:
        {
            ...
            // and fill in a table containing a list of all GLINT compatible 
            // devices.
            //
            ...
            if (! pDev->pGLInfo) {
                ...
                if (Regs->CRS.Client_ECX == MINIVDD_SHAREGLINFO)  {
                    ...
                    InitializeHardware(pDev, 0);
                }
            } else {

                dsBase = ReinitialiseDataSegment(
                             pDev, 
                             Regs->CRS.Client_DS, 
                             WindowsVMHandle);
        ...
        case MINIVDD_INITIALISEMODE:
            ...
            if (! InitializeMode(pDev)) {
                DISPDBG(("InitializeMode failed"));
                break;
            }
        ...
        case MINIVDD_FREEMEMORY:
            ...
            //
            // As we are freeing a selector, make sure none of the client 
            // selectors are actually set to this value
            //

            if (Regs->CWRS.Client_DX == Regs->CRS.Client_DS) {
                Regs->CRS.Client_DS = 0;
            }
            if (Regs->CWRS.Client_DX == Regs->CRS.Client_ES) {
                Regs->CRS.Client_ES = 0;
            }
            if (Regs->CWRS.Client_DX == Regs->CRS.Client_FS) {
                Regs->CRS.Client_FS = 0;
            }
            if (Regs->CWRS.Client_DX == Regs->CRS.Client_GS) {
                Regs->CRS.Client_GS = 0;
            }
        ...
        case MINIVDD_VMIREQUEST:
        {
            ...
            //
            // We must first translate the 16 bit memory passed in into 
            // a 32 bit pointer...
            ...
            // Work out the base and the limit of the selector
            pInput = (LWord >> 16) | 
                     ((HWord & 0xff) << 16) | (HWord & 0xff000000);
            pInput += OffsetIn;
            DISPDBG(("Input pointer: 0x%x", pInput));
            
            //    
            // If the input selector is the same as the output, then don't 
            // bother calling VMM to do the conversion again. Simply apply
            // the new offset
            //
            ...
            break;
        } //End of case MINIVDD_VMIREQUEST:
        ...
        case MINIVDD_I2CREQUEST:
        {
            ...
            if (DoI2C(pDev, (LPI2CREQUEST)pInput, (LPI2CREQUEST)pOutput)) {
                return (1);
            }

            break;
        } //End of case MINIVDD_I2CREQUEST:
    
        case MINIVDD_MEMORYREQUEST:
        {
            extern DWORD _stdcall VXD3DL_MemoryRequest(
                                      DWORD             dwDDB, 
                                      DWORD             hDevice, 
            ...
            if (ClientIn == Regs->CRS.Client_DS) {
                Regs->CRS.Client_DS = 0;
            }
            if (ClientIn == Regs->CRS.Client_ES) {
                Regs->CRS.Client_ES = 0;
            }
            if (ClientIn == Regs->CRS.Client_FS) {
                Regs->CRS.Client_FS = 0;
            }
            if (ClientIn == Regs->CRS.Client_GS) {
                Regs->CRS.Client_GS = 0;
            }
    
            if (ClientOut != ClientIn) {

                if (ClientOut == Regs->CRS.Client_DS) {
                    Regs->CRS.Client_DS = 0;
                }
                if (ClientOut == Regs->CRS.Client_ES) {
                    Regs->CRS.Client_ES = 0;
                }
                if (ClientOut == Regs->CRS.Client_FS) {
                    Regs->CRS.Client_FS = 0;
                }
                if (ClientOut == Regs->CRS.Client_GS) {
                    Regs->CRS.Client_GS = 0;
                }
            }
        ...
        case MINIVDD_ENABLEVGA:

            //            
            // Called by driver's Disable function
            ...
            //
            // disable interrupt on device, unvirtualize interrupt,
    ...
    DWORD           LoadType)
{
    ...
    if (! pDev) {   
        ...
        //
        // This may be because it has already been enumerated. 
        // Check to see if its in the table.
        //
    ...
    if (! pDev) {
        ...
        if (CM_Read_Registry_Value(
            ...
            //
            // PNP_NewDevNode for device which isnt a 3dlabs host vga chip
            //
    ...
    //
    // Initialize this device as a secondary. 
    // We may not have a valid pDev though.
    //

    DISPDBG(("Initialising as secondary device"));    
    InitializeSecondaryDispatchTable();
    ...
    PDEVICETABLE    pDev)
{
    ...
    if (intrFlags != 0) {
        ...
        //
        // Check for VBlank. On a P3 we want to change the the cursor                   // under a VBLANK
        // 
        ...
        if (intrFlags & INTR_VBLANK_SET)  {
            ...
            // 
            // Top bit of Cursor control will be set 
            // if we have to update cursors.
            //

            if (pDev->pGLInfo->wCursorMode & 0x8000) {

                pP3RDRAMDAC pP3RDRegs;

                pP3RDRegs = 
                    (pP3RDRAMDAC)&(pDev->pRegisters->Glint.ExtVCReg);
                pDev->pGLInfo->wCursorMode &= 0xFF;   // Remove update bit
                P3RD_LOAD_INDEX_REG(
                    P3RD_CURSOR_MODE, pDev->pGLInfo->wCursorMode);
            }
        ...
        if (intrFlags & (INTR_DMA_SET|INTR_ERROR_SET)) {
            ...
            //
            // We may return without starting any DMA and hence not expecting 
            // any interrupt so clear the InterruptPending flag. This will force
            // the display driver to wake us. MUST DO THIS BEFORE CHECKING THE
            // QUEUE. Since the display driver adds entries and then checks the
            // flag, we must do it in the reverse order.
            //
            ...
            if (pDev->pGLInfo->frontIndex != backIndex) {
                ...
                //
                // Since we know we'll get a DMA interrupt, we don't need a 
                // wakeup so set the InterruptPending flag to true.
                //

                pDev->pGLInfo->InterruptPending = 1;

                LOAD_GLINT_CTRL_REG(
                    DMAAddress, pDev->pGLInfo->QAddress[backIndex]);
                LOAD_GLINT_CTRL_REG(
                    DMACount, pDev->pGLInfo->QCount[backIndex]);
