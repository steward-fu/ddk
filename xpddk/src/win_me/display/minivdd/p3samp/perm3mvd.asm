...
;
; This file contains the PERMEDIA specific VGA support and provides the 
; additional functionality required to support windowed DOS 4 plane graphics
; mode windows.
;
; Usually this is implemented by reserving an off-screen portion of the 
; framebuffer as a scratch pad for converting between 4 plane graphics and
; the HiRes packed pixel format of the visible screen.  Unfortunately, the 
; PERMEDIA VGA core can only reach the first 512K of the framebuffer and only
; 64K when non 640 pixel per line modes are in effect (because banking is
; not active in these modes). Therefore the VGA core cannot be guaranteed 
; access to the off-screen area of all Hi-Res modes. To solve this the VGA
; core is allocated the first 64 Kbytes of the framebuffer and all Windows
; Hi-Res modes should start 64 Kbytes beyond the framebuffer start.
;
...
page +
Declare_Virtual_Device  PERM3MVD,               \
                        3,                      \
                        1,                      \
                        MiniVDD_Control,        \
                        Undefined_Device_ID,    \
...
;
;   Added temporarily for debugging purpose
;
...
; Store the DevNode of the chip we are running on. (for Registry reads/writes)
        ...
        invoke  InitializeDynamicInit, eax
        xor     ebx, ebx
        mov     HWInitialized, ebx
;
...
subttl          InitializePrimaryDispatchTable
page +
public  _InitializePrimaryDispatchTable
BeginProc _InitializePrimaryDispatchTable, RARE
...
EndProc _InitializePrimaryDispatchTable


subttl          InitializeSecondaryDispatchTable
page +
public  _InitializeSecondaryDispatchTable
BeginProc _InitializeSecondaryDispatchTable, RARE
...
ISDT_ErrorExit:
        ...
        xor     eax, eax
        ret
EndProc _InitializeSecondaryDispatchTable
