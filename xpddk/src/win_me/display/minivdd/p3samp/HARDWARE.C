    ...
    char       *str)
{
    ...
    while (*str != 0) {
        ...
        if (pre[index] == 0) {
            ...
            //
            // We matched the entire prefix in the string. 
    ...
    DWORD          *pnDevices)
{
    ...
    //
    // Get PCI bus location information
    //
    ...
    DISPDBG((
        "PCI Bus 0x%x, Slot 0x%x, Function 0x%x", 
        pDev->PCIBus, pDev->PCIDevice, pDev->PCIFunction));
    DISPDBG(("Glint Device Found. Checking Log Conf"));

    //
    // Retrieve PCI configuration from the PCI bus driver
    //
    ...
    //
    // Disaster in configuration manager, just nuke the dev table and bail out
    //

    if ((pDev->PCIConfig.VendorID != 0x3d3d) && 
        (pDev->PCIConfig.VendorID != 0x104c)) {
    ...
    DISPDBG((
        "SubSystemVendorID = 0x%04x, SubSystemID = 0x%04x",
         pDev->PCIConfig.u.type0.SubSystemVendorID, 
        pDev->PCIConfig.u.type0.SubSystemID));

    pDev->ChipID     = pDev->PCIConfig.DeviceID;
    pDev->RevisionID = pDev->PCIConfig.RevisionID | pDev->ChipID << 16;
    pDev->FamilyID   = PERMEDIA3_ID;

    //
    // Get CM Config information
    //

    CM_Get_Alloc_Log_Conf(
        &(pDev->CMConfig), 
        DevNode, 
        0);
    ...
    if (pDev->CMConfig.wNumMemWindows == 0) {
        ...
        DISPDBG((
            "Normal configuration not available, resorting to boot config"));
        CM_Get_Alloc_Log_Conf(
            &(pDev->CMConfig), 
            DevNode, 
            1);
    ...
    //
    // Retrieve information (Register initialize values) from the ROM
    // Afer PCI bus setup is done.
    //
    ...
    if (pDev->CMConfig.wNumMemWindows == 0) {
        ...
        DISPDBG((
            "NOTE: Some PCI Chipsets will cause delta boards to crash if\n"));
        DISPDBG(("they write to the PCI space using bytes rather than words."));
        ...
        while (ulValue < 6) {
            ...
            pDev->CMConfig.dMemBase[ulValue] = 
                pDev->PCIConfig.u.type0.BaseAddresses[ulValue];
        ...
        for (ulValue = 0; 
             ulValue < pDev->CMConfig.wNumMemWindows; 
             ulValue++) {

            pDev->CMConfig.dMemLength[ulValue] = 
                1 + ~pDev->PCIConfig.u.type0.BaseAddresses[ulValue];
            pDev->PCIConfig.u.type0.BaseAddresses[ulValue] = 
                pDev->CMConfig.dMemBase[ulValue];                      
            
            DISPDBG((
                "Regions %d from PCI: Addr 0x%x Len 0x%x", ulValue,
                pDev->CMConfig.dMemBase[ulValue], 
                pDev->CMConfig.dMemLength[ulValue])); 
        }
    ...
    while (ulValue < MAX_MEM_REGISTERS) {
        ...
        pDev->CMConfig.dMemBase[ulValue - nToGo]   = 
            pDev->CMConfig.dMemBase[ulValue];
        pDev->CMConfig.dMemLength[ulValue - nToGo] = 
            pDev->CMConfig.dMemLength[ulValue];
        pDev->CMConfig.wMemAttrib[ulValue - nToGo] = 
            pDev->CMConfig.wMemAttrib[ulValue];
    ...
    PDEVICETABLE    pDevTable)
{
    ...
    //
    // Before this is called, InitializeHardwareNode will either have found 
    // a glint device or not. Whatever the case, it will have initialised the
    // Chip field in pDevTable[0] Set up nDevices to avoid overwriting this
    // first node.
    //
    ...
    CLIENT_STRUCT  *Regs)
{
    ...
    DISPDBG((
        "TurnVGAOn DevNode 0x%x, EDX 0x%x", DevNode, Regs->CRS.Client_EDX));
    DISPCHIP((pDev));
