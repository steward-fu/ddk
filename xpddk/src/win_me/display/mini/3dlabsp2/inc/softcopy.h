...
#if (defined(_MSDOS) || defined(__cplusplus))
...
typedef struct 
{
    ...
    union
    {
        __GlintWindowFmat           Window;
        __GigiWindowFmat            PermediaWindow;
    };

    // Alpha/Fog registers
    __GlintAlphaTestModeFmat        AlphaTestMode; // Not Permedia

    union 
    {
        __GlintAlphaBlendModeFmat   GlintAlphaBlendMode;
        __GigiAlphaBlendModeFmat    PermediaAlphaBlendMode;
    };

    // The below units are used when we are chromakeying on Glint
    // This involves the AlphaBlendMode and the AlphaTestMode registers.
    // The AlphaTestMode does the 'keying' and the AlphaBlendMode smoothes
    // off the edges of the blend to get rid of edge artifacs while bilinearing
    // ChromaKeyed textures.
    __GlintAlphaBlendModeFmat       ChromaAlphaBlendMode;
    __GlintAlphaTestModeFmat        ChromaAlphaTestMode;

    __GlintFogModeFmat              FogMode;

    // Delta Register
    __GlintDeltaModeFmat            DeltaMode;

    // Common Texture Registers
    __GlintTextureColorModeFmat     TextureColorMode;

    // Glint Texture Registers
    __GlintTextureAddressModeFmat   GlintTextureAddressMode;
    __GlintTextureReadModeFmat      GlintTextureReadMode;
    __GlintTextureFormatFmat        GlintTextureFormat;

    // New MX registers.
    __GlintTextureFilterModeFmat    GlintTextureFilterMode;
    __GlintChromaTestModeFmat       GlintChromaTestMode;

    __GigiYUVModeFmat               PermediaYUVMode;
    // Gigi and Permedia Texture Registers
    union
    {
        __GigiTextureAddrModeFmat   GigiTextureAddressMode;
        __GigiTextureAddrModeFmat   PermediaTextureAddressMode;
    };
    union
    {
        __GigiTextureReadModeFmat   GigiTextureReadMode;
        __GigiTextureReadModeFmat   PermediaTextureReadMode;
    };
    union
    {
        __GigiTextureDataFormatFmat GigiTextureDataFormat;
        __GigiTextureDataFormatFmat PermediaTextureDataFormat;
    };
    union
    {
        __GigiTextureMapFormatFmat  GigiTextureMapFormat;
        __GigiTextureMapFormatFmat  PermediaTextureMapFormat;
    };

    // Total 23 DWORDS : 92 Bytes
} __GlintSoftwareCopy;
