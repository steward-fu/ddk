    ...
    LPVOID          lpStuff)
{
    ...
    if (pGLInfo->dwDeviceHandle == 0) {
        ...
        if (lpDeviceName) {
            ...
            if (pGLInfo->dwDeviceHandle == 0 || 
                pGLInfo->dwDeviceHandle == 0xFFFFFFFF) {
            ...
            strcpy(pGLInfo->szDeviceName, lpDeviceName);

        } else {

            pGLInfo->dwDeviceHandle = 1;
            strcpy(pGLInfo->szDeviceName, "DISPLAY");
    ...
    if (f) {
        ...
        if (style == InquireInfo) {
            ...
            //
            // leave memory access on if style == EnableDevice
            //
    ...
    LPVOID      lpGDIINFO)
{
    ...
    if (pGLInfo->dwDeviceHandle == 1) {
        ...
        return (ReEnable1(lpDevice, lpGDIINFO));
    }

    if (HWEnable(ENABLE_ALL) != ENABLE_ERROR) {

        bRet = ReEnable1(lpDevice, lpGDIINFO);
    }
...
UINT FAR PASCAL _loadds 
ValidateMode(
    DISPVALMODE FAR    *pdvm)
{
    UINT                result = 0;
    DWORD               dw;

    DPF(
        "ValidateMode : mode = %dx%d,%dBpp", 
        pdvm->dvmXRes, 
        pdvm->dvmYRes, 
        pdvm->dvmBpp);
    ...
    DWORD FAR  *lpPalette)
{
    ...
    DPF("SetPalette");
    if (pGLInfo->dwDeviceHandle == 1) {
        return (SetPalette1(start, count, lpPalette));
    }
    ...
    LPVOID          lpOutput)
{
    ...
    if (pGLInfo->dwDeviceHandle == 1) {
        return (Control1(lpDevice, function, lpInput, lpOutput));
    }
...
//------------------------------------------------------------------------------------------
static UINT FAR PASCAL _loadds 
CreatePhysSel(
    DWORD       PhysAddress, 
    DWORD       PhysSize)
{
