...
typedef struct tagThunkedData
{
    ...
    // Flag to tell stretchblits whether to filter. Set as the occasion demands,
    // but usually agrees with g_DXGlobals.dwFilterStretch. But, if you want to
    // set it up for a special task (e.g. emulating video overlays), you can.
    ...
    ULONG_PTR       StartDMA;                        // Pointer to the StartDMA function
    DWORD           PartitionSize;
    DWORD           BufferLocked;
    GLDMABUFF       DMAInfo;
    ...
    HINSTANCE       hInstance;
#else
    DWORD           hInstance;      // Handles are 16bit in Win 16
#endif
