...
use strict;
    ...
    sub Version          {return("v. 2.2")};    # Current version
    my $DEBUG = 0;
...
sub ProcInf {
    ...
    print("\tCreating new inf ($inffile)\n") if ($DEBUG);
    ...
    my ($hFile, @Lines, $CopyFiles);
    @Lines = ReadUnicodeOrANSIFile("$inffile");
    # Total number of lines
    $TotalLines = @Lines + 1;
    close(hFILE);
    # Do any files get copied?
    $CopyFiles = 0;
    $CopyFiles = grep(/copyfiles/i,@Lines);
    ...
    if ( grep(/^SourceDisksNames/i, $CurrentINF->getSections()) ) {
        unless ( grep(/^SourceDisksFiles/i, $CurrentINF->getSections() ) ) {
            AddErrorRef(1003, 0, 'SourceDisksFiles') if (($CopyFiles) && (! $INTERNAL));
        }
    } else {
        AddErrorRef(1003, 0, 'SourceDisksNames') if (($CopyFiles) && (! $INTERNAL));
    }
    ...
    if ( $CurrentINF->sectionDefined("DestinationDirs") ) {
        ...
        AddErrorRef(1003, 0, 'DestinationDirs') if ($CopyFiles);
    }
    ...
    my @idx;
    # Found a BOM, assume unicode
    if ($uni[0] == 0xFF and $uni[1] == 0xFE) {
        @idx = (-2,0);
    # No BOM, assume not unicode
    } else {
        @idx = (-1,0);
    }
    @uni = unpack("C*",$l);
    # looking for \r\n
    AddWarnRef(2205, 0) if ($uni[$#uni+$idx[0]] != 13 and $uni[$#uni+$idx[1]] != 10);
    undef @uni;
    undef @idx;
    undef $l;
    ...
    if ( grep(/Copyright/i,@all_lines) < 1 ) {
        AddWarnRef(2212,0);
    }
...
sub ProcClassInstall32Section {
        # HKR = HKLM,Sytem\CurrentControlSet\Control\Class\{SetupClassGUID}
...
sub ProcInstallCoInstallersSection {
        # HKR = HKLM,Sytem\CurrentControlSet\Control\Class\{%SetupClassGUID%}\{device-instance-id}
...
sub ProcInstallHWSection {
        ...
        # HKR = HKLM,Sytem\CurrentControlSet\ENUM\Model??\DevInstanceID
...
sub ProcInstallSection {
        # HKR = HKLM,Sytem\CurrentControlSet\Control\Class\{SetupClassGUID}\{DevInstanceID}
...
sub ProcInterfaceInstall32Section {
    ...
    my($count)   =     0;
    my($line);
        # HKR = HKLM,Sytem\CurrentControlSet\Control\DeviceClasses\{InterfaceClassID}
    my($Directive, $Value, @Values);
...
sub ProcInterfaceInstallSection {
    ...
    if (defined $CheckedSections{$Section})  {
        ...
        foreach $line (@lines) {
            ...
            if ((uc($Directive) eq "ADDREG") && defined($Value)){ # Must have atleast one AddReg!
                $MIN = TRUE;
            }
...
#        }
    ...
    my @GrepLines = ReadUnicodeOrANSIFile("$CurrentINF->{inffile}");
    my $cnt=grep(/^\s*CopyFiles\s*=/i,@GrepLines);
    close hFile;
...
sub ProcSDNames {
    ...
    foreach $line (@lines) {
        ...
        # Because our Get funcitons remove string tokens, we check for
    ...
    if (defined $CheckedSections{uc($Section)})  {
        ...
        foreach $line (@lines) {
            ...
            if (defined($Flags) && ($Flags !~ /^[\s\t]*$/)) {
                ...
                # if the least sigificant bit of flags is '1', then the upper 16
                ...
                # 
                # If the least signigicant bit is '0', then the upper 16 bits must
    ...
    if (defined $CheckedSections{uc($Section)})  {
        ...
        foreach $line (@lines) {
            ...
            # Check if IOCONFIG key.
                ...
                @IOList=GetList($Value,$CurrentINF->getIndex($Section, $count),1);
                $cnt=@IOList;
                for($i = 0; $i < $cnt; $i++) {
            ...
            # Check if MEMCONFIG key.
                ...
                @MEMList= GetList($Value,$CurrentINF->getIndex($Section, $count),1);
                $cnt=@MEMList;
                for($i = 0; $i < $cnt; $i++) {
                    #start-end[(attr)]
...
# RichKn -- Can a printer INF have a CMDLINE directive .....
                unless (($Values[0] >= 66000) and
                    ...
                    AddErrorRef(1044,$CurrentINF->getIndex($Section, $count),"DirID ($Values[0])",$Section)
                        unless (defined($DirIDValues{$Values[0]}));
                }
...
sub AddDeviceSpecificError {
    ...
    if (defined($ErrorTable{$LineNum})) {
        ...
        ## return(FALSE) if ($ErrorTable{$LineNum} =~ /\(DE22\.1\.$ErrorNum\)/);
        # Allow dup error numbers if the text differs
        return(FALSE) if ($ErrorTable{$LineNum} =~ /\Q(DE22.1.${ErrorNum}) $Error\E/i);
    }

    $this_str = ("(DE22.1.${ErrorNum}) $Error");
    if (defined($ErrorTable{$LineNum})) {
...
sub AddDeviceSpecificWarning {
    ...
    if (defined($WarnTable{$LineNum})) {
        ...
        ## return(FALSE) if ($WarnTable{$LineNum} =~ /\(DW22\.1\.$WarnNum\)/);
        # Allow dup warning numbers if the text differs
        return(FALSE) if ($WarnTable{$LineNum} =~ /\Q(DW22.1.${WarnNum}) $Warn\E/i);
    }

    $this_str = ("(DW22.1.${WarnNum}) $Warn");
    if (defined($WarnTable{$LineNum})) {
...
sub GetList {
    ...
    $line = RemoveStringTokens($Lnum, $line);
...
sub GetWarning {
    ...
    my(%WarningTable) = (
        ...
        2014 => ["String tokens inside quotes are interpretted literally."],
        ...
        2021 => ["%%'s in string tokens are interpretted literally."],
...
sub PrintResults {
    ...
    while(<IN>) {
        if ($. <= 1 && m/^\xFF\xFE/) { substr($_, 0, 2, '') } # remove any UNICODE BOM - leave rest of line (only needed for first line)
        if (m/\x00/) { tr/\x00//d; chop; chop; } # HACK TO HANDLE UNICODE (only for ASCII equivs)
...
sub PrintSummary {
    ...
    print(OUT "\tChkInf $Version: &copy; Copyright 1998-2001 Microsoft Corp.<BR>\n");
...
sub SplitLine {
    ...
    $Value     = RemoveStringTokens($CurrentINF->getIndex($Section, $Offset), $Value);
...
##
## Currently, this only works for UNICODE files that
## have BOM set.
##
sub ReadUnicodeOrANSIFile {
        ...
        my @data = unpack("CC",$lines[0]);

        if (($data[0] == 0xff and $data[1] == 0xfe) ||
                grep(/\x00/, @lines)){
                # is unicode w/ BOM
                # or found null bytes (illegal in ANSI)
                
                # remove just the BOM bytes
                substr ($lines[0], 0, 2, '') if ($data[0] == 0xff and $data[1] == 0xfe);
                # quick and dirty way to remove null high-order bytes.
                # valid ASCII characters should be work with this.
                map {tr/\x00//d,
                        # drop the last two bytes and add \n to work around
                        # \r\n not surviving the above tr/// as expected
                        chop, chop, $_ .= "\n"}
                        @lines;
        } else {
                # doesn't seem to be a unicode file
