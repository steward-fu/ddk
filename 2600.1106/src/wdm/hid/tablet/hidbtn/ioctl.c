    ...
    )
{
        ...
        sizeof(HID_INPUT_REPORT))
    {
        ...
        LogError(ERRLOG_INVALID_BUFFER_SIZE,
                 ...
                 NULL,
                 NULL);
        TRACEWARN(("invalid input report size (len=%d).\n",
                   irpsp->Parameters.DeviceIoControl.OutputBufferLength));
    }
    else
    {
        ...
        TRACEASSERT(OldCancelRoutine == NULL);
...
{
    TRACEPROC("ReadReportCanceled", 2)
    PDEVICE_EXTENSION devext;
    KIRQL OldIrql;

    TRACEENTER(("(DevObj=%p,Irp=%p)\n", DevObj, Irp));

    TRACEASSERT(Irp->Cancel);
    TRACEASSERT(Irp->CancelRoutine == NULL);
    ...
    IoReleaseRemoveLock(&devext->RemoveLock, Irp);
    IoCompleteRequest(Irp, IO_NO_INCREMENT);
    TRACEINFO(1, ("ReadReport IRP was canceled.\n"));
