...
{                       
    ...
    DISPDBG((
        "ShellBroadcastCallback: uMsg %x, wParam %x, lParam %x, pDev %x", 
        uMsg, wParam, lParam, pDevTable));
                ...
                case DBT_DEVICEARRIVAL:                    
                    {
                        ...
                        if (pDev == NULL) {
                            DISPDBG((
                                "WM_DEVICECHANGE WARNING: pDev %x is not a 3Dlabs device", 
                                pBroadcast->dbcd_devnode));
                            break;
                        }
