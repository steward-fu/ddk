...
//
// list has this very large 'cxcy' value (See comment in heap.h)
//
// This macro results in the available list being maintained with a
// cx-major, cy-minor sort:
//

#define CXCY(cx, cy) ((((LONG)cx) << 20) | ((LONG)cy))


//
// Global varibales for the heap manager
//
...
extern OH far *
pohValidateAndMoveOffscreenDfbToDib(
    OH far *);


#define CalculatePixelOffsetForHeap(poh) \
    (((LONG)wScreenWidth) * ((LONG)poh->y) + (LONG)poh->x + ScreenPixelOffset)

#define CalculateScreenAddrForHeap(poh) \
    ((CalculatePixelOffsetForHeap(poh)) * (wBpp >> 3))
    ...
    OH far     *poh) 
{
    ...
    Sel = *((WORD *)(&poh) + 1);
    DISPDBG((
        Value, "%4x:%x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x", 
        Sel, Off, poh->x, poh->y, poh->cx, poh->cy, poh->ofl, 
        (WORD)(poh->cxcy >> 20), (WORD)poh->cxcy));
    ...
    if (poh->pohNext && (Sel != -1)) {
        ...
        DISPDBG((
            Value, 
            "Next :%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x",
            Sel, Off, poh->pohNext->x, poh->pohNext->y, 
            poh->pohNext->cx, poh->pohNext->cy, poh->pohNext->ofl, 
            (WORD)(poh->pohNext->cxcy >> 20), (WORD)poh->pohNext->cxcy, 
            Sel1, Off1));
    ...
    if (poh->pohPrev && (Sel != -1)) {
        ...
        DISPDBG((
            Value, 
            "Prev :%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x",
            Sel, Off, poh->pohPrev->x, poh->pohPrev->y, 
            poh->pohPrev->cx, poh->pohPrev->cy, poh->pohPrev->ofl, 
            (WORD)(poh->pohPrev->cxcy >> 20), (WORD)poh->pohPrev->cxcy, 
            Sel1, Off1));
    ...
    if (poh->pohLeft && (Sel != -1)) {
        ...
        DISPDBG((
           Value, 
           "Left :%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x", 
           Sel, Off, poh->pohLeft->x, poh->pohLeft->y, 
           poh->pohLeft->cx, poh->pohLeft->cy, poh->pohLeft->ofl, 
           (WORD)(poh->pohLeft->cxcy >> 20), (WORD)poh->pohLeft->cxcy, 
           Sel1, Off1));
    ...
    if (poh->pohRight && (Sel != -1)) {
        ...
        DISPDBG((
            Value, 
            "Right:%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x",
            Sel, Off, poh->pohRight->x, poh->pohRight->y, 
            poh->pohRight->cx, poh->pohRight->cy, poh->pohRight->ofl, 
            (WORD)(poh->pohRight->cxcy >> 20), (WORD)poh->pohRight->cxcy, 
            Sel1, Off1));
    ...
    if (poh->pohUp && (Sel != -1)) {
        ...
        DISPDBG((
            Value, 
            "Up   :%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x", 
            Sel, Off, poh->pohUp->x, poh->pohUp->y, 
            poh->pohUp->cx,  poh->pohUp->cy, poh->pohUp->ofl, 
            (WORD)(poh->pohUp->cxcy >> 20), (WORD)poh->pohUp->cxcy, 
            Sel1, Off1));
    ...
    if (poh->pohDown && (Sel != -1)) {
        ...
        DISPDBG((
            Value, 
            "Down :%4x:%4x %d,%d %d x %d, Flags 0x%x, cxcy 0x%x%x OSB %4x:%4x",
            Sel, Off, poh->pohDown->x, poh->pohDown->y, 
            poh->pohDown->cx, poh->pohDown->cy, poh->pohDown->ofl, 
            (WORD)(poh->pohDown->cxcy >> 20), (WORD)poh->pohDown->cxcy, 
            Sel1, Off1));
    ...
    DISPDBG((
        Value, 
        "OSB  : %4x:%4x Base: 0x%x%x Limit: 0x%x%x", 
        Sel, Off, (WORD)(poh->OSB_Base >> 16), (WORD)poh->OSB_Base, 
        (WORD)(poh->OSB_Limit >> 16), (WORD)poh->OSB_Limit));
...
displayOH()
{
    ...
    while (h->cxcy != CXCY_SENTINEL) {
        ...
        DISPDBG((
            2, "\tAt (%4d, %4d) size = [%4d, %4d]", h->x, h->y, h->cx, h->cy));
        h = h->pohNext;
    }
    ...
    OFFSCREEN_BITMAP far   *osb)
{
    ...
    if ((Value <= DD_DebugLevel) && (osb != NULL)) {
        ...
        DISPDBG((
            Value, 
            "OFFSCREEN_BITMAP at %x:%x, PixelOffset 0x%x%x", 
            Sel, Off, 
            (WORD)(osb->OSB_PixelOffset >> 16), (WORD)osb->OSB_PixelOffset));
    ...
    OFFSCREEN_BITMAP far   *osb)
{
    ...
    //
    // Points to found available rectangle we'll use
    //
...

    {
        ...
        Off1 = *((WORD *)(&(osb)));
        Sel1 = *((WORD *)(&(osb)) + 1);
        if ((osb->OSB_Heap->ofl & OFL_AVAILABLE) || 
            (Off != Off1) || ((Sel | 7) != (Sel1 | 7))) {

            DISPDBG((
                -1, 
                "Either osb points at 'Available' bitmap or reverse pointer invalid"));
            DebugCrash();
        }
    }
    ...
    OH far     *poh)
{
    ...
    if ((poh->osb->DIB.deType != TYPE_DIBENG) || 
        (poh->osb->HostDIB.deType != TYPE_DIBENG) || 
...
MergeLoop:
    ...
    DISPDBG((1, "Freeing %d x %ld at (%d, %ld)", 
    ...
    DISPDBG((
        1, "Actually Freeing %d x %ld at (%d, %ld)", 
        poh->cx, poh->cy, poh->x, poh->y));
    ...
    FLOH        floh)           // Allocation flags
{
    ...
    DISPDBG((3, "Allocating %d x %ld...", cxThis, cyThis));
    ...
    if ((cxThis > OSB_HeapInfo.cxMax) || (cyThis > OSB_HeapInfo.cyMax)) {
        ...
        DISPDBG((
            1, "requested rect bigger than largest available (%d x %d)",
    ...
    if (pohThis->cxcy == CXCY_SENTINEL) {
        ...
        do {
            ...
            ASSERTDD(
                pohThis != &OSB_HeapInfo.ohDfb, "Ran out of in-use entries");

            if ((pohThis == NULL) || (pohThis == &OSB_HeapInfo.ohDfb)) {

                DISPDBG((
                    -1, "Very bad news on the pohThis front (0x%08X)", 
                    pohThis));
                DISPLAYHEAP();
            }
    ...
    if ((cyRem <= OH_QUANTUM) || (cxRem <= OH_QUANTUM) || 
        (pohThis->cx != (int)wScreenWidth)) {
    ...
    pohThis->osb  = NULL;    // Caller is responsible for setting this field
    ...
    DISPDBG((1, "Allocated at (%d, %ld)", pohThis->x, pohThis->y));
    DISPPOH((1, pohThis));
...
vCalculateMaximum()
{
    ...
    for (i = 2; i != 0; i--) {
        ...
        for (poh = pohSentinel->pohNext; 
             poh != pohSentinel; poh = poh->pohNext) {
    ...
    OFFSCREEN_BITMAP far   *osb)
{
    ...
    ASSERTDD(
        !(osb->DIB.deFlags & OFFSCREEN),
        "Can't move a bitmap off-screen when it's already off-screen");
    DISPOSB((10, osb));

    //
    // If offscreen is disabled for any reason, we cant move anything 
    // back into offscreen memory.
    //

    if ((! OffscreenEnabled) ||
        (osb->DIB.deHeight <= 8 && osb->DIB.deWidth <= 8) ||
        (osb->DIB.deHeight > wScreenHeight) || 
        (osb->DIB.deWidth > wScreenWidth))
        
        return (FALSE);

    poh = pohAllocate(
              osb->DIB.deWidth, osb->DIB.deHeight,
              FLOH_ONLY_IF_ROOM);
    ...
    OH far                 *poh)
{
    ...
    BitBlt16(
        ...
        0, 0, 
        &(osb->HostDIB), 
        0, 0, osb->DIB.deWidth, osb->DIB.deHeight, BLACKNESS, NULL, NULL);
#endif
    ...
    OFFSCREEN_BITMAP far   *osb)
{
    ...
    DISPDBG((
        1, 
        "CreateOffscreenBitmap %d x %d", 
        osb->HostDIB.deHeight, osb->HostDIB.deWidth));
    ...
    if (poh != NULL) {
        ...
        DISPDBG((
            1, "DrvCreateDeviceBitmap failed for size %d x %d", 
            osb->DIB.deWidth, osb->DIB.deHeight));
    ...
    if ((osb) && (osb->DIB.deFlags & OFFSCREEN)) {
        ...
        if (! (osb->OSB_Heap->ofl & OFL_PERMANENT)) {
            ...
            BitBlt16(
                ...
                0, 0, 
                &(osb->HostDIB), 
                0, 0, osb->DIB.deWidth, osb->DIB.deHeight, 
                BLACKNESS, NULL, NULL);
#endif
...
DisableOffscreenHeap()
{
    ...
    if ((pGLInfo->DisabledByGLDD) || 
        ((pGLInfo->dwFlags & GMVF_OFFSCRNBM) == 0)) {

        DISPDBG((
            -1, 
            "  disableFlags = %x, flag = %x", 
            pGLInfo->DisabledByGLDD, (pGLInfo->dwFlags & GMVF_OFFSCRNBM)));
    ...
    //
    // Check whether the heap has already been disabled
    //

    if (! OffscreenEnabled) {

        DISPDBG((-1, "Offscreen heap has already been disabled"));
        return;
    }
    ...
    //
    // Make sure all our offscreen bitmaps get 
    // converted back to Host memory bitmaps
    //
    ...
    WORD        cyScreen)
{
    ...
    cyMemory = (WORD)((pGLInfo->ddFBSize - pGLInfo->dwScreenBase) / 
                      wScreenWidthBytes);

    ScreenPixelOffset = pGLInfo->dwScreenBase / (pGLInfo->dwBpp >> 3);

    DISPDBG((
        0, "Screen: %d x %d  Memory: %ld x %ld",
    ...
    poh->x        = 0;
    poh->y        = 0;
    poh->cx       = (WORD)cxMemory;         // Width is normally less than 32768
    poh->cy       = cyMemory;
    ...
    DISPDBG((
        10, 
        "bEnableOSB_HeapInfo: screen OH is at 0x%x", 
        &OSB_HeapInfo.ohDfb));
    ...
    if (poh && 
        (pGLInfo->dwRenderFamily == PERMEDIA_ID) &&
        ...
        //
        // Reserve the Font Cache. We can decide exactly how much to allocate 
        // here. We want to allocate the cache at the end of the offscreen to
        // enhance performance, so preallocate the earlier scanlines and free
        // them
        //

        cyFonts = (WORD) ((pGLInfo->dwFontCacheSize << 12) / 
                          pGLInfo->dwScreenWidthBytes);
        ...
        if (RegOpenKey(
            ...
            if (RegQueryValueEx(
                ...
                DISPDBG((
                    -1, 
                    "Registry setting OS FontCache to %d lines", 
                    cyFonts));
            }
...
#endif
                        pGLInfo->dwOSFontCacheStart = 
                            CalculateScreenAddrForHeap(poh);
#if DEBUG
                    }
#endif
                } else {
        ...
        DISPDBG((
            -1, 
            "Allocated %ld lines to OS FontCache (%ld KBytes)", 
            cyFonts, (cyFonts * pGLInfo->dwScreenWidthBytes) / 1024));
