...
BOOL _DX_LIN_InitializeHeapManager(LinearAllocatorInfo* pAlloc);
void _DX_LIN_UnInitializeHeapManager(LinearAllocatorInfo* pAlloc);
DWORD _DX_LIN_GetFreeMemInHeap(LinearAllocatorInfo* pAlloc, 
                               LinearAllocatorCallbackFn *pCallbackFn() );
...
typedef DWORD MemoryMap[]; // Chunks memory map

typedef struct tagHashTable HashTable;

typedef struct tagLinearAllocatorInfo
{
    DWORD bResetLinAllocator;         // Bool to signal us the allocators
    ...
    DWORD dwMemPerChunk;              // How much heap memory each chunk
                                      //  controls
    HashTable *pHTAllocatedMem;       // Ptr to Hash table which keeps
                                      //  sizes allocated to each request 
