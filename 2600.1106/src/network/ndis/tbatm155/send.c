...
{                           	
   ...
   NDIS_PHYSICAL_ADDRESS_UNIT  PhysicalAddressArray[NIC_MAX_PHYSICAL_BUFFERS];
   UINT                        PacketLength;
   PMAP_REGISTER               pMapRegister;
   UINT                        c;
...
#else  // else of NEW_DMA

   //
   //	Get the first packet buffer and the number of physical
   //	segments that comprise the packet.
   //
   NdisQueryPacket(Packet, &PhysicalBufferCount, NULL, &Buffer, &PacketLength);
   ...
   //
   //	Init the total posted Tx pending slots of this packet.
   //
   Reserved->PhysicalBufferCount = 0;

   //
   //	Get information on the first ndis buffer.
   //
   while (NULL != Buffer)
   {
       //
       //	Get the virtual address and length of the current buffer.
       //
       NdisQueryBuffer(Buffer, NULL, &BufferLength);

       //
       //	Map the ndis buffer.
       //
       //  Update Available Map registers.
       //  Accumulate the number of the Map registers used by the packet.
       //
       pXmitDmaQ->FreeMapRegisters--;
       Reserved->NumOfUsedMapRegisters++;

       RemoveMapRegisterFromHead(
           &pXmitDmaQ->FreeMapRegisterQ,
           &pMapRegister);

#if DBG

       if (NULL == pMapRegister)
       {
		    DBGPRINT(DBG_COMP_SEND, DBG_LEVEL_ERR,
			    ("TbAtm155TransmitPacket: pMapRegister == NULL.\n"));
           DBGBREAK(DBG_COMP_SEND, DBG_LEVEL_ERR);
       }

#endif // end of DBG

       InsertMapRegisterAtTail(
           &pXmitSegInfo->InUseMapRegisterQ,
           pMapRegister);
       
       NdisMStartBufferPhysicalMapping(
           pAdapter->MiniportAdapterHandle,
           Buffer,
           (ULONG)pMapRegister->MapRegisterIndex,
           TRUE,
       ...
       //
       //  TxSlotNum was initialized in the beginning of the routine.
       //
       for (c = 0; c < PhysicalBufferCount; c++)
       {
           //
           //	Initialize the descriptor with the physical buffer info.
           //  and write to Tx_Pending_slots registers of 155 PCI SAR 
           //
           TxPendingSlotCtrlReg.Slot_Size = PhysicalAddressArray[c].Length;
           ...
           TBATM155_WRITE_PORT(
               &TbAtm155Sar->Tx_Pending_Slots[TxSlotNum].Base_Addr,
               NdisGetPhysicalAddressLow(PhysicalAddressArray[c].PhysicalAddress));
...
#if    DBG
           pXmitDmaQ->dbgTotalPostedTxSlots++;
#endif // end of DBG
       ...
       //	Update the total remaining Tx free slots.
       //
       pXmitDmaQ->RemainingTransmitSlots -= PhysicalBufferCount;
...
--*/
{
   ...
   if (ARGUMENT_PRESENT(Packet))
   {
       ...
       //
#ifndef NEW_DMA
       if ((pXmitDmaQ->FreeMapRegisters < Reserved->PhysicalBufferCount) ||
           (pXmitDmaQ->RemainingTransmitSlots < Reserved->PhysicalBufferCount))
#else
...
--*/
{
...
#ifndef NEW_DMA
   CurrentFreeMapRegisters = pSar->XmitDmaQ.FreeMapRegisters;
#endif
   ...
   while (!PACKET_QUEUE_EMPTY(&pXmitSegInfo->SegWait))
   {
       ...
       //	Are there transmit DMA resources available?
       //
       if (CurrentFreeMapRegisters < Reserved->PhysicalBufferCount)
       {
           //
           //  Too bad, there is no resources available.
           //
       ...
       {
           CurrentFreeMapRegisters -= (ULONG)Reserved->PhysicalBufferCount;
       }
#endif
...
--*/
{
        ...
        i = (i == pHwInfo->MaxIdxOfTxReportQ)? 0 : ++i)
   {
...
#ifndef NEW_DMA
       //
       //	Walk the buffers of the packet and free up the map registers
       //	that were used. If the map register is zero then the
       //	packet was slave copied down and map registers were not used.
       //
       NdisQueryPacket(
           Packet, 
           &PhysicalBufferCount, 
       ...
       while (Buffer != NULL)
       {
           ...
           if ((Reserved->NumOfUsedMapRegisters == 0) || (NULL == pMapRegister))
           {
               DBGPRINT(DBG_COMP_SEND, DBG_LEVEL_ERR,
                   ("No more Map register used by this packets\n"));

               // 
               // BooBoo!! We should never get here. 
               // 
               Buffer = NULL;
               continue;
           }
           ...
           //
           //	Get the next buffer and the map register that
           //	was used for it.
           //
           NdisGetNextBuffer(Buffer, &Buffer);
       ...
       NdisInterlockedIncrement(&pAdapter->StatInfo.XmitPdusOk);
...
--*/
{
   ...
   do {
            ...
            c++, CurrentPkt++)
       {
           ...
           PXMIT_SEG_INFO              pXmitSegInfo;
           PXMIT_DMA_QUEUE             pXmitDmaQ = &pVc->HwInfo->SarInfo->XmitDmaQ;
           PPACKET_RESERVED            Reserved;
           PMEDIA_SPECIFIC_INFORMATION pMediaInfo;
           ULONG                       SizeMediaInfo;
           PATM_AAL_OOB_INFO           pAtmOob;
...
#ifndef NEW_DMA
           Reserved->NumOfUsedMapRegisters = 0;
#endif
           Reserved->PhysicalBufferCount = (UCHAR)PhysicalBufferCount;
	...
	while (NULL != pErrorPacket)
	{
		...
		NdisInterlockedIncrement(&pVc->Adapter->StatInfo.XmitPdusError);
		NdisInterlockedIncrement(&pVc->StatInfo.XmitPdusError);
   ...
   )
{
   ...
   (PUCHAR)Map->Va = (PUCHAR)Map->AllocVa + AlignmentOffset;
   ...
   do 
   {
       ...
       if (NULL != VirtualAddress)
       {
           ...
           for (i = 0; i < TBATM155_MAX_PADTRAILER_BUFFERS; i++)
           {
               ...
               //
               (PUCHAR)pXmitSegInfo->PadTrailerBuffers[i].AllocVa = 
