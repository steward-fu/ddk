...
--*/
{
   ...
   //	Align the block.
   //
   Offset = gDmaAlignmentRequired -
               (ULONG)((ULONG_PTR)pRecvPool->VirtualAddress % gDmaAlignmentRequired);
...
--*/
{
   ...
   //
   Offset = gDmaAlignmentRequired -
               (ULONG)((ULONG_PTR)VirtualAddress % gDmaAlignmentRequired);
...
--*/
{
   ...
   *pAllocatedRecvBuffers += (SHORT) pRecvPool->BuffersFreed;
...
--*/
{
   ...
   PRECV_BUFFER_QUEUE          pFreeSmallBufQ = &pSar->FreeSmallBufferQ;
   PRECV_BUFFER_QUEUE          pFreeBigBufQ = &pSar->FreeBigBufferQ;
   PRECV_BUFFER_QUEUE          pFreeBufQ;
...
#if    DBG
   PRECV_BUFFER_HEADER         pTmpBuf;
   PULONG                      pdbgTotalUsedRxSlots;
   PRECV_BUFFER_QUEUE          pDbgBufferQ;
   ULONG                       dbgTmp;
#endif // end of DBG
        ...
        i = (i == pHwInfo->MaxIdxOfRxReportQ)? 0 : ++i)
   {
       ...
       if (NULL == pVc)
       {
           ...
           NdisInterlockedIncrement(&pAdapter->StatInfo.RecvInvalidVpiVci);
       ...
       if (pEntryOfRxReportQ->RxReportQDWord1.Bad)
       {
           ...
           switch (pEntryOfRxReportQ->RxReportQDWord1.Status)
           {
               case Crc32ErrDiscard:
                   ...
                   NdisInterlockedIncrement(&pVc->StatInfo.RecvCrcError);
                   NdisInterlockedIncrement(&pAdapter->StatInfo.RecvCrcError);
                   break;

               case LengthErrDiscard:
                   DBGPRINT(DBG_COMP_RECV, DBG_LEVEL_ERR,
                       ("Packet Length Error detected, discard packet.\n"));
                   NdisInterlockedIncrement(&pVc->StatInfo.RecvPdusError);
                   NdisInterlockedIncrement(&pAdapter->StatInfo.RecvPdusError);
				    break;
               ...
               case SlotCongDiscard:
                   ...
                   NdisInterlockedIncrement(&pVc->StatInfo.RecvCellsDropped);
                   NdisInterlockedIncrement(&pAdapter->StatInfo.RecvCellsDropped);
				    break;

               case OtherCellErrDiscard:
                   DBGPRINT(DBG_COMP_RECV, DBG_LEVEL_ERR,
                       ("Other Cell Error detected, discard packet.\n"));
                   NdisInterlockedIncrement(&pVc->StatInfo.RecvPdusNoBuffer);
                   NdisInterlockedIncrement(&pAdapter->StatInfo.RecvPdusNoBuffer);
				    break;

               case ReassemblyTimeout:
                   DBGPRINT(DBG_COMP_RECV, DBG_LEVEL_ERR,
                       ("Reassembly Timeout detected, discard packet.\n"));
                   NdisInterlockedIncrement(&pVc->StatInfo.RecvReassemblyErr);
                   NdisInterlockedIncrement(&pAdapter->StatInfo.RecvReassemblyErr);
				    break;
