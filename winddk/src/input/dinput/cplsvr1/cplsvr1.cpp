...
HRESULT CDIGameCntrlPropSheet_X::Initialize(void)
{
    ...
    LPTSTR   ptszBuf = new (TCHAR[STR_LEN_64]);
	assert (ptszBuf);

    // load the sheet caption
    LoadString(ghInst, IDS_SHEETCAPTION, ptszBuf, MAX_PATH);
#ifndef UNICODE
    // compiled for ANSI
    //
    // need to convert the string to UNICODE
    MultiByteToWideChar(CP_ACP, MB_PRECOMPOSED, ptszBuf, lstrlen(ptszBuf),
                        m_wszSheetCaption, sizeof(WCHAR) * MAX_PATH);
#else
    lstrcpy(m_wszSheetCaption, ptszBuf);
#endif
