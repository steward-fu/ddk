...
//       ("==>tbAtm155CompletePacketQueue\n"));
	...
	while (!PACKET_QUEUE_EMPTY(PacketQ))
	{
       ...
       while (Reserved->NumOfUsedMapRegisters)
       {
       ...
       DBGPRINT(DBG_COMP_RESET, DBG_LEVEL_INFO,
           ("FreeMapRegisters= %ld (TotalMapRegisters(%ld)\n",
             pXmitDmaQ->FreeMapRegisters, pXmitDmaQ->TotalMapRegisters));
#endif // end of NEW_DMA
...
--*/
{
   ...
   PRECV_BUFFER_QUEUE  pRetrunBufferQ;
   ...
   do {
            ...
            Link = Link->Flink)
       {
           ...
           if (NULL == pVc)
           {
	            DBGPRINT(DBG_COMP_RESET, DBG_LEVEL_ERR, ("pVc==NULL\n"));
