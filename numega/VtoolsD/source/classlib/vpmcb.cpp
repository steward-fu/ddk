// vpmcb.cpp - implementation of class VProtModeCallback 
//=============================================================================
//
...
extern "C" PVOID __cdecl createCallbackThunk(VProtModeCallback*,BYTE*,BOOL(VProtModeCallback::*)(VMHANDLE,CLIENT_STRUCT*,PVOID));

LISTHANDLE VProtModeCallback::m_list = 0;


// Function
//	VProtModeCallback::VProtModeCallback
//
// Syntax
//	VProtModeCallback::VProtModeCallback(PVOID refData)
//
// Parameters
...
// If there is none, allocate a new one.
	...
	else 
	{
		...
		m_dwCBAddr = Allocate_PM_Call_Back(refData, (PPMCall_HANDLER) pFunc, NULL);
...
//
// Syntax
//	VProtModeCallback::~VProtModeCallback(VOID)
//
// Remarks
...
BOOL VProtModeCallback::handler(VMHANDLE hVM, CLIENT_STRUCT* pRegs, PVOID refData)
{
	// if this function is called, it means you have not correctly
	// overridden member function handler in the class you derived
	// from VProtModeCallback.

	DEBUGEVENT(DBG_ERROR,"\"handler\" for VProtModeCallback class is not defined correctly");
