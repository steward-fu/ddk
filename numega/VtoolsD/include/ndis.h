...
#define NdisDprAllocatePacketNonInterlocked NdisAllocatePacket
#define NdisDprFreePacket					NdisFreePacket
#define NdisDprFreePacketNonInterlocked		NdisFreePacket
...
// Their drivers
// VtoolsD_REMOVE: typedef NDIS_STATUS NTSTATUS; // "Them" is probably us?
typedef CCHAR KIRQL;
typedef KIRQL *PKIRQL;
#define HIGH_LEVEL 31
#define PDRIVER_OBJECT PVOID
// VtoolsD_REMOVE: #define PUNICODE_STRING PVOID
...
typedef struct _WRAPPER_MAC_BLOCK      NDIS_MAC_BLOCK, * PNDIS_MAC_BLOCK;
typedef struct _WRAPPER_ADAPTER_BLOCK  NDIS_ADAPTER_BLOCK, * PNDIS_ADAPTER_BLOCK;
typedef struct _WRAPPER_PROTOCOL_BLOCK NDIS_PROTOCOL_BLOCK, * PNDIS_PROTOCOL_BLOCK;
typedef struct _WRAPPER_OPEN_BLOCK     NDIS_OPEN_BLOCK, * PNDIS_OPEN_BLOCK;
typedef struct _MODULE_LIST            NDIS_WRAPPER_HANDLE, * PNDIS_WRAPPER_HANDLE;
...
//
// Used by NDIS WAN protocol to send packets
// to miniports.  No macro version is available
// at this point.
...
typedef KEVENT NDIS_EVENT, *PNDIS_EVENT;

//
// Event must be initialized before use.
//
VOID NDIS_API
NdisInitializeEvent(
...
//
// Signals the event to unblock a thread waiting on this.
//
...
//
// Resets an event to a unsignalled state
//
VOID NDIS_API
NdisResetEvent(
    IN  PNDIS_EVENT Event
...
//
// Waits for the event to be signalled or MsToWait whichever is sooner.
// Returns TRUE if event is signalled, false if timed-out
//
BOOLEAN NDIS_API
NdisWaitEvent(
    IN  PNDIS_EVENT Event,
...
Begin_Service_Table Ndis
    ...
    Ndis_Service NdisMWanSend, Local
    Ndis_Service DbgPrint, Local

    Ndis_Service NdisInitializeEvent, Local
    Ndis_Service NdisSetEvent, Local
    Ndis_Service NdisResetEvent, Local
    Ndis_Service NdisWaitEvent, Local
