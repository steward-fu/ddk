...
// Handle DEVICE_INIT (static load)

BOOL XHelloDevice::OnDeviceInit(VMHANDLE hVM, PCHAR szCmdTail)
{
	SayHello();
	return TRUE;
}
...
// Handle SYS_DYNAMIC_DEVICE_INIT (dynamic load)

BOOL XHelloDevice::OnSysDynamicDeviceInit(void)
{
	SayHello();
	return TRUE;
}
...
// Handle SYS_DYNAMIC_DEVICE_EXIT (dynamic unload)

BOOL XHelloDevice::OnSysDynamicDeviceExit(void)
{
	SayGoodbye();
	return TRUE;
}
