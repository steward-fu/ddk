...
NDIS_OID WdmVNICAdapter::sm_OID_GEN_SUPPORTED_LIST[] = {
	...
	OID_GEN_SUPPORTED_GUIDS,				// wmi support
	...
	OID_CUSTOM_RW_ULONG,
	OID_CUSTOM_RO_ULONG,
	OID_CUSTOM_MAC_ADDRESS,
	OID_CUSTOM_STRING,

};

//
// WMI support (see WdmVNIC.mof)
// GUID to OID map
//
const NDIS_GUID WdmVNICAdapter::sm_OID_GEN_SUPPORTED_GUIDS[] = 
{
    {
        WDMVNIC_RW_ULONGGuid,
        OID_CUSTOM_RW_ULONG,
        sizeof(ULONG),
#ifdef fNDIS_GUID_ALLOW_READ
...
BEGIN_OID_QUERY_MAP(WdmVNICAdapter)
	...
	OID_QUERY_ENTRY( OID_CUSTOM_RW_ULONG)
	OID_QUERY_ENTRY( OID_CUSTOM_RO_ULONG)
	OID_QUERY_ENTRY( OID_CUSTOM_MAC_ADDRESS)
	OID_QUERY_ENTRY( OID_CUSTOM_STRING)
...
BEGIN_OID_SET_MAP(WdmVNICAdapter)
	...
	OID_SET_ENTRY( OID_GEN_MACHINE_NAME)
	#endif
	...
	OID_SET_ENTRY( OID_CUSTOM_RW_ULONG)
...
_inline_ NDIS_STATUS  WdmVNICAdapter::getOID_GEN_SUPPORTED_GUIDS(PNDIS_GUID pGuid, PULONG ByteCount, PULONG BytesNeeded)
{
	NDIS_STATUS status = NDIS_STATUS_SUCCESS;

	*BytesNeeded = sizeof(sm_OID_GEN_SUPPORTED_GUIDS);

	if ( *ByteCount >= sizeof(sm_OID_GEN_SUPPORTED_GUIDS) )
	{
		*ByteCount = sizeof(sm_OID_GEN_SUPPORTED_GUIDS);
		NdisMoveMemory(pGuid, sm_OID_GEN_SUPPORTED_GUIDS, sizeof(sm_OID_GEN_SUPPORTED_GUIDS));
	}
	else
	{
		*ByteCount = 0;
...
#ifdef NDIS51_MINIPORT
NDIS_STATUS  WdmVNICAdapter::setOID_GEN_MACHINE_NAME
	(WCHAR* pName, PULONG Length, OUT PULONG) 
{
	// Machine name notification (is it null-terminated?)
	
	TRACE("WdmVNICAdapter::setOID_GEN_MACHINE_NAME: %ws\n", pName);

	UNREFERENCED_PARAMETERS2(pName, Length);
	return NDIS_STATUS_SUCCESS;
}
#endif
...
////////////////////////////////////////////////////////////////////////////////////
// Custom OIDs
ULONG WdmVNICAdapter::getOID_CUSTOM_RW_ULONG()
{
	return m_nCustomSet;
}

NDIS_STATUS WdmVNICAdapter::setOID_CUSTOM_RW_ULONG(PULONG pSet)
{
	m_nCustomSet = *pSet;
...
ULONG WdmVNICAdapter::getOID_CUSTOM_RO_ULONG()
{
	return ++m_nCustomSet;
}

NDIS_STATUS WdmVNICAdapter::getOID_CUSTOM_MAC_ADDRESS(PUCHAR pBuffer, PULONG ByteCount, OUT PULONG BytesNeeded)
{
	NDIS_STATUS status = NDIS_STATUS_SUCCESS;

	*BytesNeeded = sizeof(m_CurrentAddress);

	if ( *ByteCount >= sizeof(m_CurrentAddress) )
	{
		*ByteCount = sizeof(m_CurrentAddress);
		NdisMoveMemory(pBuffer, &m_CurrentAddress, sizeof(m_CurrentAddress));
	}
	else
	{
		*ByteCount = 0;
...
NDIS_STATUS WdmVNICAdapter::getOID_CUSTOM_STRING(PUCHAR pBuffer, PULONG ByteCount, OUT PULONG BytesNeeded)
{
	NDIS_STATUS status = NDIS_STATUS_SUCCESS;

	static const char SampleString[] = "WDMVNIC WMI Sample String";

	*BytesNeeded = sizeof(SampleString);

	if ( *ByteCount >= sizeof(SampleString) )
	{
		*ByteCount = sizeof(SampleString);
		NdisMoveMemory(pBuffer, SampleString, sizeof(SampleString));
	}
	else
	{
		*ByteCount = 0;
