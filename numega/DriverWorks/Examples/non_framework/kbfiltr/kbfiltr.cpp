...
--*/
{
    ...
    case IOCTL_INTERNAL_I8042_HOOK_KEYBOARD:
...
#if (USE_CPP_EXCEPTIONS)		
		//Make sure the hookKeyboard structure can be read.
		// ProbeForRead raises an exception if the pointer is invalid
		try{
			ProbeForRead(hookKeyboard,
				sizeof(INTERNAL_I8042_HOOK_KEYBOARD),
				1);
		}
		catch(KException)
		{
			DbgPrint("Caught KException\n");
			status = STATUS_INVALID_PARAMETER;
			break;	
		}
		catch(...)
		{
			...
			//We will get here if KException::Install was not called
			status = STATUS_INVALID_PARAMETER;
			break;
		}
#endif		
...
--*/
{
    ...
    devExt = static_cast<PDEVICE_EXTENSION>(DeviceObject->DeviceExtension);
...
--*/
{
	...
	process_scan(InputDataStart, InputDataEnd);
}
