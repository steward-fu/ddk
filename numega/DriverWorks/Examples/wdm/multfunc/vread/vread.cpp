...
KDebugOnlyTrace t("VRead");

/////////////////////////////////////////////////////////////////////
// Begin INIT section
#pragma code_seg("INIT")

DECLARE_DRIVER_CLASS(VRead, NULL)
...
//			registry.  To locate VRead look for:
//			HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\VRead
//
//	Return Value:
...
NTSTATUS VRead::DriverEntry(PUNICODE_STRING RegistryPath)
{
...
NTSTATUS VRead::AddDevice(PDEVICE_OBJECT Pdo)
{
	t << "AddDevice called\n";

	VReadDevice* pDevice = new (
			static_cast<PCWSTR>(KUnitizedName(L"VReadDevice", m_Unit)),
			FILE_DEVICE_UNKNOWN,
			NULL,
			0,
			DO_BUFFERED_IO | DO_POWER_PAGABLE
			)
		VReadDevice(Pdo, m_Unit);
	...
	if ( pDevice  )
	{
		...
		{
			t << "Error constructing device VReadDevice " << m_Unit << " status " << (ULONG)status << EOL;
			delete pDevice;
		}
	}
	else
	{
		t << "Error creating device VReadDevice " << m_Unit << EOL;
