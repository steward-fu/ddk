...
//
// Define length of interupt object dispatch code in longwords.
//

#define DISPATCH_LENGTH 3*2               // Length of dispatch code plable in 32-bit words
...
//
// **** TBD change if this are made intrinsics
...
//
// Define the page size for the IA64 as 4096 (0x1000)
//
...
#define MmLockPagableCodeSection(PLabelAddress) \
    MmLockPagableDataSection((PVOID)(*((PULONGLONG)PLabelAddress+1)))
...
KeBugCheckEx(
    ...
    IN UINT_PTR BugCheckParameter1,
    IN UINT_PTR BugCheckParameter2,
    IN UINT_PTR BugCheckParameter3,
    IN UINT_PTR BugCheckParameter4
    );
...
#if defined(_ALPHA_) || defined(_IA64_) || (defined(_X86_) && defined(_NTHAL_))
...
#if defined(_ALPHA_) || defined(_MIPS_) || defined(_IA64_)
...
#define PAGE_ALIGN(Va) ((PVOID)((UINT_PTR)(Va) & ~(PAGE_SIZE - 1)))
...
#define ADDRESS_AND_SIZE_TO_SPAN_PAGES(Va,Size) \
   ...
   (((((ULONG)(Size-1)&(PAGE_SIZE-1)) + ((UINT_PTR)Va & (PAGE_SIZE -1)))) >> PAGE_SHIFT)) + 1L)

#define COMPUTE_PAGES_SPANNED(Va, Size) \
    ((((UINT_PTR)Va & (PAGE_SIZE -1)) + (Size) + (PAGE_SIZE - 1)) >> PAGE_SHIFT)
...
#define MmGetMdlVirtualAddress(Mdl)                                     \
    ...
    (PVOID) ((UINT_PTR) (Mdl)->StartVa << PAGE_SHIFT) : (Mdl)->StartVa) +  \
    (Mdl)->ByteOffset))
...
//
// Define PNP/POWER types required by IRP_MJ_PNP_POWER.
//
