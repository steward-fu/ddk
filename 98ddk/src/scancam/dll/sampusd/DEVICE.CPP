...
RunNotifications(VOID)
{
    ...
    // changes in it.
    //
    lstrcpy(szDirPath,m_pszDeviceNameA);
    pszLastSlash = strrchr(szDirPath,'\\');
    if (pszLastSlash) {
        *pszLastSlash = '\0';
    }

    hNotifyFileSystemChange = FindFirstChangeNotification(
                                szDirPath,
                                FALSE,
    ...
    )
{
    ...
    if ( GetFileAttributesEx(m_pszDeviceNameA,GetFileExInfoStandard, &sNewFileAttributes)) {
