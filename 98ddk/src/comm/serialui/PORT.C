...
#define DEFAULT_FLOWCTL             CMD_FLOWCTL_XONXOFF
...
UINT WINAPI FeFiFoFum(HWND hwndOwner, LPCSTR pszPortName);
...
char const FAR c_szSettings[] = "Settings";
char const FAR c_szComxFifo[] = "Fifo";
char const FAR c_szEnh[] = "386Enh";
char const FAR c_szSystem[] = "system.ini";
...
void GetSetAdvSettings(
    ...
    if (FindDevice(c_szPortClass, pszPortName, &hkeyDevice, &hkeyDriver) ||
        FindDeviceByString(c_szPortClass, c_szPortName, pszPortName, &hkeyDevice, &hkeyDriver) ||
        FindDeviceByString(c_szModemClass, c_szPortName, pszPortName, &hkeyDevice, &hkeyDriver)) 
        {
        switch (action)
            {
        case ACT_GET:
            ...
            if (ERROR_SUCCESS != RegQueryValueEx(hkeyDevice, c_szSettings, NULL,
                NULL, (LPSTR)&settings, &cbData)) 
                {
        ...
        case ACT_SET:
            ...
            RegSetValueEx(hkeyDevice, c_szSettings, NULL, REG_BINARY,
                (LPSTR)&settings, sizeof(SETTINGS));
            break;
        ...
        cbData = 250;       // leave room for "fifo" on the end
        RegQueryValueEx(hkeyDevice, c_szPortName, NULL, NULL, szFifo,
            &cbData);
...
BOOL CALLBACK AdvPort_DlgProc(
    ...
    case WM_COMMAND:
        pszPortName = (LPCSTR)GetWindowLong(hDlg, DWL_USER);
        if (!pszPortName)
            {
            ASSERT(0);
            break;
