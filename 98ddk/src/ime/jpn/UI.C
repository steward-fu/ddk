    ...
    if (!hUICurIMC)
    {
        switch (message)
        {
            ...
            case WM_IME_CHAR:
                ...
                {
                char szDev[80];
                OutputDebugString((LPSTR)"Why hUICurIMC is NULL????\r\n");
                wsprintf((LPSTR)szDev,"\thWnd is %x\r\n",hWnd);
                OutputDebugString((LPSTR)szDev);
                wsprintf((LPSTR)szDev,"\tmessage is %x\r\n",message);
                OutputDebugString((LPSTR)szDev);
                wsprintf((LPSTR)szDev,"\twParam is %x\r\n",wParam);
                OutputDebugString((LPSTR)szDev);
                wsprintf((LPSTR)szDev,"\tlParam is %x\r\n",lParam);
                OutputDebugString((LPSTR)szDev);
                }
#endif
                return 0L;
