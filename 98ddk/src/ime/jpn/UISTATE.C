...
LPARAM lParam;
{
    ...
    switch (message)
    {
        ...
        case WM_CREATE:
            hbmpStatus = LoadBitmap(hInst,"STATUSBMP");
            SetWindowLong(hWnd,FIGWL_STATUSBMP,(LONG)hbmpStatus);
            hbmpStatus = LoadBitmap(hInst,"CLOSEBMP");
...
void PASCAL ButtonStatus( HWND hStatusWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        ...
        case WM_RBUTTONUP:
            ...
            hMenu = LoadMenu(hInst, "RIGHTCLKMENU");
            if (hMenu && (hIMC = (HIMC)GetWindowLong(hSvrWnd,IMMGWL_IMC)))
            {
                int cmd;
                POINT pt;
                ...
                switch (cmd)
                {
                    case IDM_RECONVERT:
                    {
                        DWORD dwSize = MyImmRequestMessage(hIMC, IMR_RECONVERTSTRING, 0);
                        if (dwSize)
                        {
                            ...
                            if (dwSize = MyImmRequestMessage(hIMC, IMR_RECONVERTSTRING, (LPARAM)lpRS)) {
...
#ifdef DEBUG
{
    ...
    *(LPMYSTR)(lpDump + lpRS->dwStrLen) = MYTEXT('\0');
    ...
    OutputDebugString("IMR_RECONVERTSTRING\r\n");
    wsprintf(szDev, "dwSize            %x\r\n", lpRS->dwSize);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwVersion         %x\r\n", lpRS->dwVersion);
    OutputDebugString(szDev);
    ...
    OutputDebugString("\r\n");
                                ...
                                OutputDebugString("ImmRequestMessage returned 0\r\n");
#endif
