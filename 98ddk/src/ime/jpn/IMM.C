...
{
    ImeLog(LOGF_ENTRY | LOGF_API, "ImeInquire");
...
{
    ImeLog(LOGF_API, "ImeConversionList");
...
{
    ImeLog(LOGF_ENTRY | LOGF_API, "ImeDestroy");
...
{
    ImeLog(LOGF_API, "ImeSetActiveContext");
...
BOOL WINAPI ImeProcessKey(HIMC hIMC,UINT vKey,LPARAM lKeyData,CONST LPBYTE lpbKeyState)
{
    ...
    ImeLog(LOGF_KEY | LOGF_API, "ImeProcessKey");
...
BOOL WINAPI NotifyIME(HIMC hIMC,DWORD dwAction,DWORD dwIndex,DWORD dwValue)
{
    ...
    switch(dwAction)
    {
             ...
             if (IsConvertedCompStr(hIMC))
             {
                 ...
                 if (lpCandInfo = (LPCANDIDATEINFO)ImmLockIMCC(lpIMC->hCandInfo))
                 {
                     ...
                     nBufLen = GetPrivateProfileString(GETLPCOMPREADSTR(lpCompStr),
                            NULL,(LPSTR)"",
                            (LPMYSTR)szBuf,256,(LPSTR)szDicFileName );
#endif
        ...
        case NI_IMEMENUSELECTED:
             ...
             {
             char szDev[80];
             OutputDebugString((LPSTR)"NotifyIME IMEMENUSELECTED\r\n");
             wsprintf((LPSTR)szDev,"\thIMC is 0x%x\r\n",hIMC);
             OutputDebugString((LPSTR)szDev);
             wsprintf((LPSTR)szDev,"\tdwIndex is 0x%x\r\n",dwIndex);
             OutputDebugString((LPSTR)szDev);
             wsprintf((LPSTR)szDev,"\tdwValue is 0x%x\r\n",dwValue);
             OutputDebugString((LPSTR)szDev);
             }
#endif
             break;
...
BOOL WINAPI ImeSelect(HIMC hIMC, BOOL fSelect)
{
    ...
    ImeLog(LOGF_ENTRY | LOGF_API, "ImeSelect");
...
void DumpRS(LPRECONVERTSTRING lpRS)
{
    ...
    *(lpDump + lpRS->dwStrLen) = MYTEXT('\0');

    OutputDebugString("DumpRS\r\n");
    wsprintf(szDev, "dwSize            %x\r\n", lpRS->dwSize);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwStrLen          %x\r\n", lpRS->dwStrLen);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwStrOffset       %x\r\n", lpRS->dwStrOffset);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwCompStrLen      %x\r\n", lpRS->dwCompStrLen);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwCompStrOffset   %x\r\n", lpRS->dwCompStrOffset);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwTargetStrLen    %x\r\n", lpRS->dwTargetStrLen);
    OutputDebugString(szDev);
    wsprintf(szDev, "dwTargetStrOffset %x\r\n", lpRS->dwTargetStrOffset);
    OutputDebugString(szDev);
...
{
    ImeLog(LOGF_API, "ImeSetCompositionString");

    switch (dwIndex)
    {
        case SCS_QUERYRECONVERTSTRING:
#ifdef DEBUG
            OutputDebugString("SCS_QUERYRECONVERTSTRING\r\n");
            if (lpComp)
...
#ifdef DEBUG
            OutputDebugString("SCS_SETRECONVERTSTRING\r\n");
            if (lpComp)
                DumpRS((LPRECONVERTSTRING)lpComp);
            if (lpRead)
                DumpRS((LPRECONVERTSTRING)lpRead);
...
{
    ImeLog(LOGF_API, "ImeGetImeMenuItems");
    ...
    if (!lpImeParentMenu)
    {
        ...
        else
        {
            ...
            lpImeMenu->hbmpItem = LoadBitmap(hInst,"FACEBMP");
    ...
    else
    {
        ...
        else
        {
            ...
            lpImeMenu->hbmpChecked = LoadBitmap(hInst,"CHECKBMP");
            lpImeMenu->hbmpUnchecked = LoadBitmap(hInst,"UNCHECKBMP");
