    ...
    LPPOINT lpptWnd)            // the composition window position
{
    ...
    if (ptNew.x + sImeG.xCandWi > sImeG.rcWorkAreaCa.right) {
        // exceed screen width
        ptNew.x = lpptWnd->x - sImeG.xCandWi - UI_MARGIN;
    }

    ptNew.y = lpptWnd->y + lpImeL->cyCompBorder - sImeG.cyCandBorder;
    if (ptNew.y + sImeG.yCandHi > sImeG.rcWorkAreaCa.bottom) {
        // exceed screen high
        ptNew.y = sImeG.rcWorkAreaCa.bottom - sImeG.yCandHi;
    }
    ...
    LPPOINT lpptWnd)            // the composition window position
{
    ...
    if(lpptWnd->y + ptFontHi + sImeG.yCandHi <= sImeG.rcWorkAreaCa.bottom) {
    	lpptWnd->y += ptFontHi;
	} else {
    ...
    LPPOINT        lpptCandWnd)            // the caret position
{
    ...
    *(LPPOINT)&rcUIRect = *lpptCandWnd;
    ...
    if (rcUIRect.left < sImeG.rcWorkAreaCa.left) {
        rcUIRect.left = sImeG.rcWorkAreaCa.left;
    } else if (rcUIRect.left + sImeG.xCandWi > sImeG.rcWorkAreaCa.right) {
        rcUIRect.left = sImeG.rcWorkAreaCa.right - sImeG.xCandWi;
    } else {
    }

    if (rcUIRect.top < sImeG.rcWorkAreaCa.top) {
        rcUIRect.top = sImeG.rcWorkAreaCa.top;
    } else if (rcUIRect.top + sImeG.yCandHi > sImeG.rcWorkAreaCa.bottom) {
        rcUIRect.top = sImeG.rcWorkAreaCa.bottom - sImeG.yCandHi;
    } else {
    }
    ...
    *(LPPOINT)&rcUIRect = *lpptCandWnd;
    ...
    if (uEsc & 0x0001) {
        ...
        *lpptCandWnd = *(LPPOINT)&rcUIRect;
        ...
        if (rcUIRect.top < sImeG.rcWorkAreaCa.top) {
        } else if (rcUIRect.top + sImeG.yCandHi > sImeG.rcWorkAreaCa.right) {
        } else {
            if (rcUIRect.left < sImeG.rcWorkAreaCa.left) {
                rcUIRect.left = sImeG.rcWorkAreaCa.left;
            } else if (rcUIRect.left + sImeG.xCandWi > sImeG.rcWorkAreaCa.right) {
...
{
    if (lpptCandWnd->x < sImeG.rcWorkAreaCa.left) {
        lpptCandWnd->x = sImeG.rcWorkAreaCa.left;
    } else if (lpptCandWnd->x + sImeG.xCandWi > sImeG.rcWorkAreaCa.right) {
        lpptCandWnd->x = sImeG.rcWorkAreaCa.right - sImeG.xCandWi;
    }

    if (lpptCandWnd->y < sImeG.rcWorkAreaCa.top) {
        lpptCandWnd->y = sImeG.rcWorkAreaCa.top;
    } else if (lpptCandWnd->y + sImeG.yCandHi > sImeG.rcWorkAreaCa.bottom) {
        lpptCandWnd->y = sImeG.rcWorkAreaCa.bottom - sImeG.yCandHi;
    }
    ...
    if (!lpUIPrivate->hCandWnd) {
	    ...
	    if(nShowCandCmd == SW_HIDE) {
			...
			if(uOpenCand) {
				...
				if(ptSTWPos.x + sImeG.xStatusWi + Comp_CandWndLen > sImeG.rcWorkAreaCa.right) {
    ...
    LPPOINT lpCursor)
{
    ...
    hIMC = (HIMC)GetWindowLong(GetWindow(hCandWnd, GW_OWNER), IMMGWL_IMC);
    if (!hIMC) {
        return;
    }
    ...
    UINT uCandDownUp)
{
    ...
    // draw button down
	hDC = GetDC(hCandWnd);
    hCandHpBmp = LoadBitmap(hInst, "CandHp");
    hCandUpBmp = LoadBitmap(hInst, "CandUp");
    hCandDpBmp = LoadBitmap(hInst, "CandDp");
    hCandEpBmp = LoadBitmap(hInst, "CandEp");
    ...
    LPARAM lParam)
{
    ...
    if (HIWORD(lParam) == WM_LBUTTONDOWN) {
    	...
    	hCandHBmp = LoadBitmap(hInst, "CandH");
    	hCandUBmp = LoadBitmap(hInst, "CandU");
    	hCandDBmp = LoadBitmap(hInst, "CandD");
    	hCandEBmp = LoadBitmap(hInst, "CandE");
    	hMemDC = CreateCompatibleDC(hDC);
    ...
    DrawDragBorder(hCandWnd, MAKELONG(ptCursor.x, ptCursor.y),
        GetWindowLong(hCandWnd, UI_MOVE_OFFSET),sImeG.rcWorkAreaCa);
    ...
    HWND hCandWnd)
{
    ...
    DrawDragBorder(hCandWnd, lTmpCursor, lTmpOffset, sImeG.rcWorkAreaCa);
    ...
    HDC  hDC)
{
    ...
    if (sImeG.IC_Trace) {
	    ...
	    for (i = 0; dwStart < dwEnd; dwStart++, i++) {
			...
			if(sImeL.dwRegImeIndex == INDEX_GB || sImeL.dwRegImeIndex == INDEX_GBK){
				...
				if (iLen > 10 * sizeof(WCHAR) / sizeof(TCHAR)) {
					iLen = 10 * sizeof(WCHAR) / sizeof(TCHAR);
	       ...
	       // QW/GB info
	       {
	         ...
	         BYTE  AbSeq[5];
	         BYTE  GbSeq[5];
			 TCHAR szMyStrBuf[10 * sizeof(WCHAR) / sizeof(TCHAR)];
			 RECT  GBARInfo;

	         wCode = *(LPUNAWORD)((LPBYTE)lpCandList + lpCandList->dwOffset[dwStart]);
			 MultiByteToWideChar(936, (DWORD)NULL, (LPSTR)&wCode, 2, &UniSeq, 1);
			 			 
	         wCode = HIBYTE(wCode) | (LOBYTE(wCode) << 8);

	         wsprintf (GbSeq,"%04lx",wCode);
			
	         wCode -= 0xa0a0;

	         wsprintf (AbSeq,"%02d%02d",HIBYTE(wCode),LOBYTE(wCode));
	                                          // get AreaPosition string
	    ...
	    for (i = 0; dwStart < dwEnd; dwStart++, i++) {
			...
			if(!GetTextExtentPoint(hDC, (LPCSTR)AnsiStr, lstrlen(AnsiStr), &StrSize))
			...
			if(!GetTextExtentPoint(hDC, (LPCSTR)szStrBuf, lstrlen(szStrBuf), &StrSize))
				memset(&StrSize, 0, sizeof(SIZE));
			nX += StrSize.cx;
    ...
    // load all bitmap
	if (sImeG.IC_Trace) {
		hCandInfBmp = LoadBitmap(hInst, "Candinf");
	} else {
	    hCandInfBmp = NULL;
	}
    ...
    hCandDBmp = LoadBitmap(hInst, "CandD");
    hCandEBmp = LoadBitmap(hInst, "CandE");
    hCandIconBmp = LoadBitmap(hInst, "CandSel");
    ...
    case WM_MOUSEMOVE:
        if (GetWindowLong(hCandWnd, UI_MOVE_OFFSET) != WINDOW_NOT_DRAG) {
...

			{
				...
				info.cbSize=sizeof(MONITORINFO);
				GetMonitorInfo(hMonitor,&info);
				sImeG.rcWorkAreaCa=info.rcWork;
			}
