/**********************************************************************/
/* Copyright (C) 1995-1996 Microsoft Corp. & ZY corp.                 */
...
BOOL WINAPI ImeInquire(         // initialized data structure of IME
    ...
    LPCTSTR   lpszOptions)      // this is for 3.1 IME only, chicago IME
                                // should not use this
{
    if (!lpImeInfo) {
        return (FALSE);
...
void PASCAL ReverseConversionList(HWND   hLayoutListBox)
{
    ...
    LoadString(hInst, IDS_NONE, szTmpImeName, sizeof(szTmpImeName));
    ...
    LONG lParam)
{
    ...
    case WM_COMMAND:
        switch (wParam) {
		case IDD_LAYOUT_LIST:
			{
				static BOOL bSwitchFocus;
				if (bSwitchFocus) {
    ...
    LPVOID  lpData)            // mode of dialog
{
    ...
    case IME_CONFIG_GENERAL:
        DialogBox(hInst, "ImeSet", hAppWnd, ImeSetDlgProc,);
        break;
    default:
        return (FALSE);
        break;
    ...
    UINT            uBufLen)
{
    ...
    for (i=0;i<4;i++) {
       ...
       wCode = GBEngine (lpImcP);
                wCode = HIBYTE(wCode) | (LOBYTE(wCode) << 8);
                for (i = 0; i < (int)MAX_COMP;i++, wCode++) {
...
BOOL AreaToGB (
        ...
        BYTE    GbSeq[5])
{
        BYTE    MbSeq[3]; // Temp string
        // Area turn
        wsprintf (MbSeq,"%lx",(AbSeq[0] * 10 + AbSeq[1]+0xa0));
        GbSeq[0] = MbSeq[0];
        GbSeq[1] = MbSeq[1];
        //position turn
        wsprintf (MbSeq,"%lx",(AbSeq[2] * 10 + AbSeq[3]+0xa0));
        GbSeq[2] = MbSeq[0];
        GbSeq[3] = MbSeq[1];
        GbSeq[4] = '\0';
        return TRUE;
    ...
    UINT            uBufLen)
{
    ...
    DWORD  dwSize =         // similar to ClearCand
        ...
        sizeof(DWORD) * MAX_COMP +
        // string plus NULL terminator
        (sizeof(BYTE) * nMaxKey + sizeof(TCHAR));
...

	{
		...
		MultiByteToWideChar(936, 0, (LPCTSTR)&wCode, sizeof(WORD), szWideStr, sizeof(szWideStr));
    ...
    lstrcpy((LPBYTE)lpCandList + lpCandList->dwOffset[0],AbSeq);
    ...
    UINT            uBufLen)
{
    ...
    lpCandList->dwOffset[1] =
     lpCandList->dwOffset[0] + 4 + sizeof(TCHAR);

    lstrcpy((LPBYTE)lpCandList + lpCandList->dwOffset[1],GbSeq);
    ...
    LPINPUTCONTEXT lpIMC)
{
    if (lpIMC->fdwInit & INIT_STATUSWNDPOS) {
        ...
        if (ptWnd.x < sImeG.rcWorkAreaSt.left) {
            lpIMC->ptStatusWndPos.x = sImeG.rcWorkAreaSt.left;
        } else if (ptWnd.x + sImeG.xStatusWi > sImeG.rcWorkAreaSt.right) {
    ...
    if (fSelect) {
        ...
        *(LPDWORD)lpImcP->bSeq = 0;
        ...
        if (!(lpIMC->fdwInit & INIT_CONVERSION)) {
            lpIMC->fdwConversion = IME_CMODE_SYMBOL | IME_CMODE_NATIVE;
	    ...
	    // set Caps status
	    {
		    ...
		    if (lpbKeyState[VK_CAPITAL] & 0x01) {
				...
				//set chinese symbol as default state
...
char   szStr[256];
	...
	LoadString(hInst,wMsgID,szStr, sizeof(szStr));
