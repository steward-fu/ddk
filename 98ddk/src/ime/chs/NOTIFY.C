...
UINT PASCAL UnicodeProcessKey(WORD,LPPRIVCONTEXT);
WORD PASCAL UnicodeEngine(LPPRIVCONTEXT);
    ...
    DWORD               dwReadLen)
{
   ...
   if(sImeL.dwRegImeIndex == INDEX_GB){
       ...
       for (i = 0; i < (DWORD)MAX_COMP;i++, wCode++) {
    ...
    if (lpCandList->dwCount == 1) {
                lstrcpy((LPBYTE)lpCompStr + lpCompStr->dwResultStrOffset,
                (LPBYTE)lpCandList + lpCandList->dwOffset[0]);

	        // calculate result string length
                lpCompStr->dwResultStrLen =
                lstrlen((LPBYTE)lpCandList + lpCandList->dwOffset[0]);
    ...
    DWORD       dwValue)
{
    ...
    case NI_COMPOSITIONSTR:
        switch (dwIndex) {
        case CPS_COMPLETE:
							      // we can not decide a code for winime
            break;              // need to handle it
