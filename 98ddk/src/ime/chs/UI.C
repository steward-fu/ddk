    ...
    LPINPUTCONTEXT lpIMC)
{
    ...
    if (lpUIPrivate->hCompWnd) {
        ...
        if (rcWnd.left > sImeG.rcWorkAreaKe.right / 2) {
            xPortion = 1;
        } else {
            xPortion = 0;
        }

        if (rcWnd.top > sImeG.rcWorkAreaKe.bottom / 2) {
            yPortion = 1;
        } else {
            yPortion = 0;
        }
    ...
    for (nPortion = 3, fPortionTest = 0x0008; fPortionTest;
        ...
        if (nPortion % 2) {
            lpIMC->ptSoftKbdPos.x = sImeG.rcWorkAreaKe.right -
                (rcWnd.right - rcWnd.left) - UI_MARGIN;
        } else {
            lpIMC->ptSoftKbdPos.x = sImeG.rcWorkAreaKe.left;
        }

        if (nPortion / 2) {
            lpIMC->ptSoftKbdPos.y = sImeG.rcWorkAreaKe.bottom -
                (rcWnd.bottom - rcWnd.top) - UI_MARGIN;
        } else {
            lpIMC->ptSoftKbdPos.y = sImeG.rcWorkAreaKe.top;
        }
    ...
    if (!(lpIMC->fdwConversion & IME_CMODE_SOFTKBD)) {
        ...
        if (!lpUIPrivate->hSoftKbdWnd) {
            ...
            lpUIPrivate->hSoftKbdWnd =
                ImmCreateSoftKeyboard(SOFTKEYBOARD_TYPE_C1, hUIWnd, 0, 0);
        }
    ...
    LPARAM      lParam)
{
    ...
    case IMN_PRIVATE:
        ...
        case IMN_PRIVATE_DESTROYCANDWIN:
    		SendMessage(GetCandWnd(hUIWnd), WM_DESTROY, (DWORD)NULL, (DWORD)NULL);
            break;
    ...
    LPARAM lShowUI)
{
    ...
    if (fOn) {
		...
		if(RegQueryValueEx (hKey, "show status",
                     NULL,
...
SetShowStatus:
		...
		// init ime properties & reset context
		{
		    ...
		    if (retCode) {
	    		...
	    		MessageBox (lpIMC->hWnd, Buf, "警告", MB_OK | MB_ICONINFORMATION);
	    		return;
    		}
    ...
    LPARAM lParam)
{
    ...
    case WM_IME_SETCONTEXT:
        ...
        // When dialog box is opened, IME is disappeded. Change IME Z-order again. 
        if (wParam && GetWindowLong(hUIWnd, IMMGWL_IMC))
        ...
        case IMC_SETSTATUSWINDOWPOS:
            {
			    ...
			    if(!sImeG.IC_Trace) {
					...
					CopyRect(&lpIMC->cfCompForm.rcArea, &sImeG.rcWorkAreaSt);
