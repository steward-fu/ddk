    ...
    LPBYTE  lpCurr)
{
    ...
    *(LPWORD)&bBuf[2] = *(LPWORD)lpszString;
    ...
    dwPos = (lpCurr - lpUsrDicStart) / (lpImeL->nSeqBytes + 2) *
    ...
    LPBYTE  lpUsrDicLimit)
{
    ...
    lpMem = (LPBYTE)GlobalAlloc(GPTR, lpUsrDicLimit - lpCurr);
    if (!lpMem) {
        return;
    }
    ...
    ReadFile(hUsrDicFile, lpMem, lpUsrDicLimit - lpCurr - lpImeL->nMaxKey - 4,
        &dwByte, NULL);

    SetFilePointer(hUsrDicFile, dwPos, (LPLONG)NULL, FILE_BEGIN);

    WriteFile(hUsrDicFile, lpMem, lpUsrDicLimit - lpCurr - lpImeL->nMaxKey - 4,
        &dwByte, NULL);
    ...
    LPSTYLEBUF  lpStyleBuf)
{
    ...
    LoadString(hInst, IDS_EUDC, lpStyleBuf->szDescription,
        sizeof(lpStyleBuf->szDescription));
