    ...
    LPPRIVCONTEXT  lpImcP)
{
    ...
    if (wCharCode == ' ' && lpImcP->iImeState == CST_INIT) {
        ...
        return (CST_INVALID);
#if defined(WINAR30)
    } else if (*(LPDWORD)lpImcP->bSeq == 0x17 && wCharCode >= '0' &&
        wCharCode <= '9') {
        return (CST_SYMBOL);
#elif defined(DAYI)
    ...
    WORD    wCharCode)
{
    if (wCharCode) {                    // one char code
        ...
        lpdwTransBuf = lpdwTransBuf + 1 + uNumMsg * 3;
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (lpImcP->fdwImeMsg & MSG_IMN_COMPOSITIONSIZE) {
            if (!i) {
                ...
                *lpdwTransBuf++ = IMN_PRIVATE_COMPOSITION_SIZE;
            }
        }
        ...
        if (lpImcP->fdwImeMsg & MSG_IMN_PAGEUP) {
            if (!i) {
                ...
                *lpdwTransBuf++ = WM_IME_NOTIFY;
                *lpdwTransBuf++ = IMN_PRIVATE;
                *lpdwTransBuf++ = IMN_PRIVATE_PAGEUP;
            }
        }
        ...
        if (lpImcP->fdwImeMsg & MSG_IMN_UPDATE_PREDICT) {
            if (!i) {
                ...
                *lpdwTransBuf++ = WM_IME_NOTIFY;
                *lpdwTransBuf++ = IMN_PRIVATE;
                *lpdwTransBuf++ = IMN_PRIVATE_UPDATE_PREDICT;
            }
        }
        ...
        if (lpImcP->fdwImeMsg & MSG_IMN_UPDATE_QUICK_KEY) {
            if (!i) {
                ...
                *lpdwTransBuf++ = WM_IME_NOTIFY;
                *lpdwTransBuf++ = IMN_PRIVATE;
                *lpdwTransBuf++ = IMN_PRIVATE_UPDATE_QUICK_KEY;
            }
        }
#else
        ...
        if (lpImcP->fdwImeMsg & MSG_IMN_TOGGLE_UI) {
            if (!i) {
                ...
                *lpdwTransBuf++ = WM_IME_NOTIFY;
                *lpdwTransBuf++ = IMN_PRIVATE;
                *lpdwTransBuf++ = IMN_PRIVATE_TOGGLE_UI;
            }
        }
        ...
        if (!i) {
            ...
            if (!lpIMC->hMsgBuf) {
#if defined(DEBUG)
                _asm int 3;             // no message buffer! for debug only
#endif
    ...
    WCHAR          wCharCode)
{
    ...
    if (!lpCompStr) {
        return TranslateImeMessage(lpdwTransBuf, lpIMC, lpImcP);
    }
    ...
    HIMC         hIMC)
{
    ...
    if (iRet == CST_ALPHANUMERIC) {
        if (lpIMC->fdwConversion & IME_CMODE_FULLSHAPE) {   // convert to DBCS
            uNumMsg = TranslateFullChar(lpdwTransBuf, lpIMC, lpImcP,
        ...
        uNumMsg = TranslateFullChar(lpdwTransBuf, lpIMC, lpImcP, wCharCode);
#endif
