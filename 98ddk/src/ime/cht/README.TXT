To build Chinese IME Samples, please just type NMAKE command
instead of BUILD command.

excutable binary files for phonetic IME -
   1. phon\phon.ime     (copy it to system directory)
   2. phon.tbl          (it is under system directory)
   3. phonptr.tbl       (it is under system directory)
   4. phoncode.tbl      (it is under system directory)
...
2. For this advance phonetic IME, what is the clause information?
   и (offset 0 to 4 in this string), 常琌 (offset 4 to 8), 
   (offset 8 to 10), and ＆＝ (oofset 10 to 14) are four clauses to
   this IME. (It may be different ways deciding a clause to other IME)
   And the clause informtion to this IME is DWORD array 0, 4, 8, 10,
   and 14.

3. What is the attribute for these three clause?
   The attribute for one clause should be the same. For this IME,
   и (ATTR_CONVERTED - 2), 常琌 (ATTR_CONVERTED - 2), 
   (ATTR_CONVERTED - 2), and ＆＝ (ATTR_INPUT - 0) can be marked
   with different attribute as following.

   и常琌＆＝
   22222222220000

   The attribute is BYE array 2,2,2,2,2,2,2,2,2,2,0,0,0,0.

4. When are the ATTR_CONVERTED and ATTR_TARGET_CONVERTED
   used?

   For example, end user keep typing -
   ...
   and end user mark 盿 (on mark time, the attribute change to
   ...
   End user press enter key to reconvert and cursor might at the end
   of the sentence -
   ...
   OK, only the clause with cursor could be ATTR_TARGET_CONVERTED.

5. If the IME do not provide the correct information. The IME aware
   application may fail to function.
...
6. On developing Win95 IME, we should alway keep in mind the
   ...
   So it is obvious to an IME designer, we should put IME UI
