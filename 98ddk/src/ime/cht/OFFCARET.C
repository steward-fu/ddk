    ...
    HWND hOffCaretWnd)
{
    ...
    hUIPrivate = (HGLOBAL)GetWindowLong(GetWindow(hOffCaretWnd, GW_OWNER),
        IMMGWL_PRIVATE);
    if (!hUIPrivate) {
        return;
    }
...
#if UNICODE
        // ?????? below is not correct for NT, please reconsider it
        uStrLen =  lstrlen((LPTSTR)((LPBYTE)lpCandList +
            lpCandList->dwOffset[dwValue]));
#else
        // hack but fast way for checking position of ANSI code
        uStrLen =  lstrlen((LPTSTR)((LPBYTE)lpCandList +
            lpCandList->dwOffset[dwValue]));
#endif
    ...
    HDC    hDC)
{
    ...
    if (lpUIPrivate->nShowCandCmd != SW_HIDE) {
        ...
        for (i = 0, nChars = 0; dwStart < dwEnd; dwStart++, i++) {
...
#if defined(UNICODE)
                // add unicode 0x25A1
                *(LPWSTR)&szStrBuf[nChars] = 0x25A1;
#else
                // add big-5 0xA1BC
                *(LPWSTR)&szStrBuf[nChars] = 0xBCA1;
#endif
                lpStr = &szStrBuf[nChars];
            ...
            if ((CANDPERPAGE * 3 - nChars) < iLen) {
                ...
                iLen = CANDPERPAGE * 3 - nChars - 2;

                // string too long no string in candidate list
                CopyMemory(&szStrBuf[nChars], lpStr, iLen * sizeof(TCHAR));
                nChars += iLen;
    ...
    LPARAM lParam)
{
    ...
    case WM_LBUTTONUP:
        if (GetWindowLong(hOffCaretWnd, UI_MOVE_OFFSET) != WINDOW_NOT_DRAG) {
            ...
            ImmSetStatusWindowPos((HIMC)GetWindowLong(hUIWnd, IMMGWL_IMC),
                &ptCursor);
        } else {
            return DefWindowProc(hOffCaretWnd, uMsg, wParam, lParam);
        }
