    ...
    UINT uIndex)
{
    ...
    lpImeLinkRegWord = (LPIMELINKREGWORD)GetWindowLong(hWnd,
        GWL_IMELINKREGWORD);
    ...
    lpImeLinkRadical = (LPIMERADICALRECT)GetWindowLong(hWnd,
        GWL_RADICALRECT);
    ...
    if (uIndex < lpImeLinkRadical->nStartIME) {
        ...
        if (lpImeLinkRadical->nStartIME > (lpImeLinkRegWord->nEudcIMEs -
            lpImeLinkRadical->nPerPageIMEs)) {
            lpImeLinkRadical->nStartIME = lpImeLinkRegWord->nEudcIMEs -
            lpImeLinkRadical->nPerPageIMEs;
        }
        ...
        if (lpImeLinkRadical->hScrollWnd) {
            ...
            scInfo.cbSize = sizeof(SCROLLINFO);
            scInfo.fMask = SIF_POS;
            scInfo.nPos = lpImeLinkRadical->nStartIME;
        ...
        InvalidateRect(hWnd, &lpImeLinkRadical->rcRadical[i], FALSE);

        i = uIndex - lpImeLinkRadical->nStartIME;
    ...
    HWND hWnd)
{
    ...
    for (i = 0, nIMEs = 0; i < nLayouts; i++) {
        ...
        fRet = ImmEscape(hKL, (HIMC)NULL, IME_ESC_GET_EUDC_DICTIONARY,
            szImeEudcDic);
    ...
    if (!nIMEs) {
        LoadString(hAppInst, IDS_NOIME_TITLE, szTitle, sizeof(szTitle));
        LoadString(hAppInst, IDS_NOIME_MSG, szMessage, sizeof(szMessage));
    ...
    if (!lpImeLinkRegWord) {
        LoadString(hAppInst, IDS_NOMEM_TITLE, szTitle, sizeof(szTitle));
        LoadString(hAppInst, IDS_NOMEM_MSG, szMessage, sizeof(szMessage));
    ...
    LoadString(hAppInst, IDS_CHINESE_CHAR, szStrBuf, sizeof(szStrBuf));
    ...
    if (lpImeLinkRegWord->nEudcIMEs > nIMEs) {
        ...
        scInfo.nMin = 0;
        scInfo.nMax = lpImeLinkRegWord->nEudcIMEs - 1;
        scInfo.nPage = nIMEs;
        scInfo.nPos = 0;
        scInfo.nTrackPos = 0;
    ...
    SetWindowLong(hWnd, GWL_RADICALRECT, (LONG)lpImeLinkRadical);
    ...
    WPARAM wParam)
{
    ...
    case SB_THUMBPOSITION:
        if (lpImeLinkRadical->nStartIME == HIWORD(wParam)) {
            // we got the same value
            return;
        }

        // set to an invalid value
        lpImeLinkRadical->nStartIME = 0xFFFFFFFF;
    ...
    case WM_CREATE:
        {
            ...
            // initialize to 0
            SetWindowLong(hWnd, GWL_IMELINKREGWORD, 0L);
            SetWindowLong(hWnd, GWL_RADICALRECT, 0L);
            ...
            SetWindowLong(hWnd, GWL_IMELINKREGWORD, (LONG)lpImeLinkRegWord);
    ...
    case WM_EUDC_REGISTER_BUTTON:
        {
            ...
            lpImeRadicalRect = (LPIMERADICALRECT)GetWindowLong(hWnd,
                GWL_RADICALRECT);
    ...
    case WM_INITDIALOG:
        {
            ...
            wsprintf(&szTitle[cbString], "%4X", (UINT)lParam);
    ...
    DWORD         dwFileSize)
{
    ...
    hKL = MatchImeName(szStr, sizeof(lpIsvUsrDic->achMethodName) /
        sizeof(TCHAR));
    ...
    LoadString(hAppInst, IDS_PROCESS_FMT, szProcessFmt, sizeof(szProcessFmt));
    LoadString(hAppInst, IDS_RESULT_FAIL, szResult[0], sizeof(szResult[0]));
    LoadString(hAppInst, IDS_RESULT_SUCCESS, szResult[1], sizeof(szResult[1]));

    LoadString(hAppInst, IDS_CHINESE_CHAR, szStr, sizeof(szStr));
    ...
    HWND hWnd)
{
    ...
    szTitle = szFilter;
    iTitle = sizeof(szFilter);
    szMessage = szDirName;
    iMessage = sizeof(szDirName);
    ...
    if (dwSize != dwFileSize) {
        ...
        MessageBox(hWnd, szMessage, szTitle, MB_OK);
    } else if (lpIsvUsrDic->idCP != BIG5_CP &&
        lpIsvUsrDic->idCP != ALT_BIG5_CP) {
        ...
        MessageBox(hWnd, szMessage, szTitle, MB_OK);
    } else if (!RegisterBig5Table(hWnd, lpIsvUsrDic, dwFileSize)) {
