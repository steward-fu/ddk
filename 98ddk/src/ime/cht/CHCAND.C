    ...
    LPPRIVCONTEXT       lpImcP)
{
    ...
    UniSearchPhrasePrediction(NATIVE_CP,
    ...
    LPPRIVCONTEXT   lpImcP)
{
    ...
    for (i = 0; i < 4; i += 2) {
        ...
        uCharCode = lpImeL->wSymbol[wSymbolSet - '0'][i];

        if (!uCharCode) {
            break;
        }

        for (; uCharCode <= lpImeL->wSymbol[wSymbolSet - '0'][i + 1];) {
            AddCodeIntoCand(lpCandList, uCharCode++);

            if (LOBYTE(uCharCode) == 0x7F) {
                uCharCode += (0xA1 - 0x7F);
            } else if (LOBYTE(uCharCode) == 0xFF) {
                uCharCode += (1 + 0x40);
            } else {
            }
        }
