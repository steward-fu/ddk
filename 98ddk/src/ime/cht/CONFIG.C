    ...
    LPTSTR szMessage)       // this buffer size must >= MESSAGE_BUF_SIZE
{
    ...
    for (i = 0; i < sizeof(USRDIC) / sizeof(WORD); i++) {
        if (szMessage[i] == '\\') {
            szMessage[i] = '!';
        }
    }
    ...
    HWND   hWnd)
{
    ...
    OPENFILENAME ofn;
    TCHAR        szFilter[64];
    TCHAR        szFileName[USR_DIC_SIZE / sizeof(WORD)];
    ...
    *(LPUSRDIC)szDirName = *(LPUSRDIC)szFileName;
    ...
    HWND        hWnd)
{
    ...
    if (sizeof(USRDICIMHDR) != 256) {
        LoadString(hInst, IDS_INTERNAL_TITLE, szTitle,
            sizeof(szTitle) / sizeof(TCHAR));
        LoadString(hInst, IDS_INTERNAL_MSG, szMessage,
    ...
    if (szFileName[0] == '\0') {
        ...
        if (lRet) {
            ...
            LoadString(hInst, IDS_EUDCDICFAIL_TITLE, szTitle,
                sizeof(szTitle) / sizeof(TCHAR));
            LoadString(hInst, IDS_EUDCDICFAIL_MSG, szMessage,
                sizeof(szMessage) / sizeof(TCHAR));
    ...
    *(LPUSRDIC)szMessage = *(LPUSRDIC)szFileName;
...
/**********************************************************************/
BOOL CALLBACK ConfigDlgProc(  // dialog procedure of configuration
    HWND   hDlg,
    UINT   uMessage,
    WPARAM wParam,
    LPARAM lParam)
...
ConfigDlgStatusPosOvr:
    ...
    case WM_COMMAND:
        ...
        case IDOK:
            // Need to change the activation at first.
            // Otherwise WM_IME_NOTIFY is sent with NULL context then 
            // We can not update configure in UI.
    ...
    LPTSTR      szMessage)      // this buffer size must >= MESSAGE_BUF_SIZE
{
    ...
    if (GetLastError() == ERROR_ALREADY_EXISTS) {
        *(LPUSRDIC)szMessage = *(LPUSRDIC)szEudcDic;
...
SetUsrDicClose:
    ...
    *(LPUSRDIC)lpImeL->szUsrDic = *(LPUSRDIC)szEudcDic;
    ...
    // max EUDC chars
    hUsrDicMem = CreateFileMapping((HANDLE)0xFFFFFFFF,
    ...
    HWND        hWnd)
{
    ...
    return SetUsrDic(
        ...
        lpInstL, lpImeL,
#endif
        hWnd, ofn.lpstrFile, szFilter, szDirName);
}
#endif
