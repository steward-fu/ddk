    ...
    DWORD       fdwErrMsg)
{
    ...
    LoadString(hInst, IDS_MEM_LESS_ERR, szErrMsg, sizeof(szErrMsg));
    ...
    UINT        uWriteLen)
{
    ...
    for (lpMem = lpUsrDicMem; dwPos < dwUsrDicSize; lpMem += uWriteLen) {
        ...
        ReadFile(hUsrDicFile, lpMem, uReadLen, &dwReadByte,
            (LPOVERLAPPED)NULL);
...
OpenUsrDicFile:
    if (hUsrDicFile != INVALID_HANDLE_VALUE) {  // OK
        ...
        LoadString(hInst, IDS_FILE_OPEN_ERR, szFmtStr, sizeof(szFmtStr));
    ...
    // max EUDC chars
    lpInstL->hUsrDicMem = CreateFileMapping((HANDLE)0xFFFFFFFF,
    ...
    if (!lpInstL->hUsrDicMem) {
        ...
        if (hReadUsrDicMem != INVALID_HANDLE_VALUE) {  // OK
...
OpenDicFile:
    ...
    hMap = CreateFileMapping((HANDLE)hTblFile, NULL, PAGE_READONLY,
        0, 0, szTable);
