...
--*/
{
    ...
    DriverObject->MajorFunction[IRP_MJ_POWER]                   = HidMiniPower;
...
--*/
{
    ...
    KeInitializeSpinLock( &deviceExtension->OutputRawQueue.SpinLock );
    KeInitializeSpinLock( &deviceExtension->OutputReportQueue.SpinLock );
    InitializeListHead( &deviceExtension->OutputRawQueue.List );
    InitializeListHead( &deviceExtension->OutputReportQueue.List );

    ExInitializeNPagedLookasideList ( &deviceExtension->OutputRawQueue.MemListPool,
                                      NULL,
                                      NULL,
                                      0,
                                      sizeof(HIDMINI_RAW_OUTPUT),
                                      'QwaR',   // HidMini RAW QUEUE
                                      0 );

    ExInitializeNPagedLookasideList    (    &deviceExtension->OutputReportQueue.MemListPool,
                                            NULL,
                                            NULL,
                                            0,
                                            sizeof(HIDMINI_REPORT_OUTPUT),
                                            'QpeR',   // HidMini Report QUEUE
                                            0 );

    deviceExtension->OutputRawQueue.nElements = 0;
    deviceExtension->OutputReportQueue.nElements = 0;

    KeInitializeDpc ( &deviceExtension->HidMiniWriteDPCObject,
                      HidMiniWriteDPC,
                      (PVOID)deviceExtension );

    KeInitializeTimer( &deviceExtension->HidMiniWriteTimer );

    //
    // Put the timer in the signaled state so that HidMiniWriteReport will
    // start it.
    //
    KeSetEvent( &deviceExtension->HidMiniWriteTimer, 0, FALSE );
