...
--*/
{
    ...
    if(bExpose) {
        ...
        if (!DeviceIoControl (GamePort->File,
                              ...
                              GamePort->Hardware, bytes,
                              &bytes, NULL)) {
              printf("Error in DeviceIoctl IOCTL_GAMEENUM_EXPOSE_HARDWARE: %x", GetLastError());
              return FALSE;
        }
        printf("Hardware handle 0x%x\n",GamePort->Hardware->HardwareHandle);
    ...
    if(bRemove) {
        ...
        if (!DeviceIoControl (GamePort->File,
                              ...
                              &remove, bytes,
                              &bytes, NULL)) {
            printf("Error in DeviceIoctl IOCTL_GAMEENUM_REMOVE_HARDWARE: %x", GetLastError());
            return FALSE;
        }
    }
    return TRUE;
