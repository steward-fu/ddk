...
BOOL CALLBACK MonDlgProc (HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (iMsg)
    {
        ...
        case WM_INITDIALOG :
            ...
            if (DeviceLoaded) {
                i = SendDlgItemMessage (hDlg, IDC_MON, CB_SETCURSEL, 0, 0);
                i = SendDlgItemMessage (hDlg, IDC_MON, CB_GETCURSEL, 0, 0);
            ...
            switch (LOWORD (wParam))
            {
                ...
                case IDC_OPTION :
                    ...
                    if (HIWORD (wParam) == CBN_SELCHANGE) {
                        i = SendDlgItemMessage (hDlg, IDC_OPTION, CB_GETCURSEL, 0, 0);
                ...
                case IDC_UP :
                    ...
                    SendMessage(GetParent(g_hwndDlg), PSM_CHANGED, (UINT)g_hwndDlg, 0L);
                    break;
                ...
                case IDC_DOWN :
                    ...
                    ChangeFeature (Control[RadioControl + 7].usagepage, Control[RadioControl + 7].usage, Control[RadioControl + 7].value);
                    Control[RadioControl + 7].HasControlChanged = TRUE;
                    SendMessage(GetParent(g_hwndDlg), PSM_CHANGED, (UINT)g_hwndDlg, 0);
                    break;
...
VOID InitMonitor(HWND hDlg)
{
    ...
    i = SendDlgItemMessage (hDlg, IDC_OPTION, CB_GETCOUNT, 0, 0);
    if (CB_ERR != i) {
        while(i) {
            i = SendDlgItemMessage(hDlg, IDC_OPTION, CB_DELETESTRING, i-1, 0);
            if (CB_ERR == i){
            i = 0;
            }
        }
    ...
    if (Control[BRIGHTNESS].available == TRUE ||
        ...
        x = SendDlgItemMessage (hDlg, IDC_OPTION, CB_ADDSTRING, 0, (LPARAM) TempText);

        InitGeneralControls(hDlg);
        i = SendDlgItemMessage (hDlg, IDC_OPTION, CB_SETCURSEL, 0, 0);
