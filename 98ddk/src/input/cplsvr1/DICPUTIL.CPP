...
HRESULT DIUtilGetJoystickTypeName(short nJoystickId, LPDIRECTINPUTJOYCONFIG pdiJoyCfg, LPTSTR tszTypeName, int nBufSize)
{
    ...
    LPDIJOYCONFIG  pdijc = new (DIJOYCONFIG);
	assert (pdijc);

    // get the type name
    ZeroMemory(pdijc, sizeof(DIJOYCONFIG));
    pdijc->dwSize = sizeof(DIJOYCONFIG);

    HRESULT hRes = pdiJoyCfg->GetConfig(nJoystickId, (LPDIJOYCONFIG)pdijc, DIJC_REGHWCONFIGTYPE);
    ...
    {       
		...
		WideCharToMultiByte(CP_ACP, 0, pdijc->wszType, -1, tszTypeName, MAX_JOYSTRING, NULL, NULL);
#else
	    // we've compiled for UNICODE
		//
	    // no conversion needed
		lstrcpy(tszTypeName, pdijc->wszType);
#endif
	}
	else 
	{
...
HRESULT DIUtilGetJoystickDisplayName(short nJoystickId, LPDIRECTINPUTJOYCONFIG pdiJoyCfg, LPTSTR tszDispName, int nBufSize)
{
    ...
    HRESULT hRes = pdiJoyCfg->GetConfig(nJoystickId, pdijc, DIJC_REGHWCONFIGTYPE);
    if(SUCCEEDED(hRes))
	{
		LPDIJOYTYPEINFO  pdijti = new (DIJOYTYPEINFO);
		assert (pdijti);

	    // get the display name
		ZeroMemory(pdijti, sizeof(DIJOYTYPEINFO));
	    pdijti->dwSize = sizeof(DIJOYTYPEINFO);

		hRes = pdiJoyCfg->GetTypeInfo(pdijc->wszType, pdijti, DITC_DISPLAYNAME);
	    if(SUCCEEDED(hRes))
		{
#ifndef UNICODE
	    // we've compiled for ANSI
		//
	    // we need to convert the type name string
		WideCharToMultiByte(CP_ACP, 0, pdijti->wszDisplayName, -1, tszDispName, MAX_JOYSTRING, NULL, NULL);
#else
	    // we've compiled for UNICODE
		//
	    // no conversion needed
                                    ...
                                    CLSID* pclsid)
{
    ...
    {       
	    ...
	    hRes = pdiJoyCfg->GetTypeInfo(pdijc->wszType, pdijti, DITC_CLSIDCONFIG);
		if(SUCCEEDED(hRes))
	    {
		    // return the CLSID
			*pclsid = pdijti->clsidConfig;
	    }
		else
	    {
		    // return a NULL GUID
