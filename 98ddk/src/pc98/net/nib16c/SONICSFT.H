...
#define SONIC_NDIS_MINOR_VERSION 0x0A
...
typedef struct _SONIC_ADAPTER {
    ...
    //
    // Pointer to the beginning of the SONIC ports.
    //
    ULONG SonicPortAddress;

    //
    // Pointer to the beginning of the NESA ports.
    //
    ULONG SonicNesaPortAddress;

    //
    // NIB Window Address.
    //
    PVOID SonicWindowAddress;

    //
    // WindowAdress Base.
    //
    SONIC_PHYSICAL_ADDRESS SonicWindowAddrBase;

    //
    // if board doesn't exist, flag = FALSE
    //                                  use at  BUNGO-DP
    USHORT BordExist;

    //
    // regist temporary Receive(LOOK AHEAD)data 
    //
    UCHAR LookAHeadData[SONIC_INDICATE_MAXIMUM];
    ...
    // For synchronization timeing when shutdown.
    UCHAR SonicCloseWaitFlag;
