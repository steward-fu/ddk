...

{
    ...
    //
    BOOLEAN ViolatedConstraints = TRUE;
...

{
    ...
    if (Adapter->PacketsSinceLastInterrupt >=
        ...
        TransmitDescriptor->TransmitConfiguration = (USHORT)SONIC_TCR_PROG_INTERRUPT;
    ...
    if (Reserved->UsedSonicBuffer) {
        ...
        TransmitDescriptor->PacketSize = (USHORT)BufferDescriptor->DataLength;
        ...
        TransmitDescriptor->FragmentCount = (USHORT)CurFragment;
        TransmitDescriptor->PacketSize = (USHORT)TotalPacketLength;
...

{
    ...
    SONIC_REMOVE_END_OF_LIST((PULONG)PrevLinkPointer);               
