...
#define _SONIC_PHYSICAL_ADDRESS         USHORT
#define _UINT                           USHORT
#define _ULONG                          USHORT
...
#define SONIC_NUMBER_OF_TRANSMIT_DESCRIPTORS 16
...
#define SONIC_NUMBER_OF_LARGE_BUFFERS ((UINT)10)
...
typedef struct _SONIC_RECEIVE_RESOURCE {
    ...
    _SONIC_PHYSICAL_ADDRESS LowBufferAddress;
    _SONIC_PHYSICAL_ADDRESS HighBufferAddress;

    //
    // The number of WORDS in the receive buffer.
    //

    _UINT LowBufferWordCount;
    _UINT HighBufferWordCount;
...
typedef struct _SONIC_RECEIVE_DESCRIPTOR {
    ...
    _SONIC_PHYSICAL_ADDRESS LowPacketAddress;
    _SONIC_PHYSICAL_ADDRESS HighPacketAddress;

    //
    // Contains the RBA and packet sequence number.
    //

    _UINT SequenceNumber;
...
typedef struct _SONIC_TRANSMIT_FRAGMENT {
    ...
    _SONIC_PHYSICAL_ADDRESS LowFragmentAddress;
    _SONIC_PHYSICAL_ADDRESS HighFragmentAddress;

    //
    // The size of the fragment.
    //

    _UINT FragmentByteCount;
...
typedef struct _SONIC_CAM_FRAGMENT {
    ...
    _UINT CamAddressPort0;
    _UINT CamAddressPort1;
    _UINT CamAddressPort2;
...
{ \
    ...
    _Cfp->CamAddressPort0 = (_UINT)(((PUSHORT)Addr)[0]); \
    _Cfp->CamAddressPort1 = (_UINT)(((PUSHORT)Addr)[1]); \
    _Cfp->CamAddressPort2 = (_UINT)(((PUSHORT)Addr)[2]); \
}
...
#define SONIC_LOAD_CAM_ENABLE(_Cfp, _Ce) \
    (_Cfp)->CamEntryPointer = (_ULONG)(_Ce)
...
#define SONIC_SET_END_OF_LIST(Plink) \
    (_ULONG)*(Plink) = (_ULONG)((*(Plink) | SONIC_END_OF_LIST))
...
{ \
    ...
    _Tdf->LowFragmentAddress = (_SONIC_PHYSICAL_ADDRESS)_Adr; \
    _Tdf->HighFragmentAddress = (_SONIC_PHYSICAL_ADDRESS)(SONIC_GET_HIGH_PART_ADDRESS(_Adr)); \
}
...
#define SONIC_SET_TRANSMIT_FRAGMENT_LENGTH(Tdf,Len) \
    (Tdf)->FragmentByteCount = (_UINT)(Len)
...
#define SONIC_SET_TRANSMIT_LINK(Tdf,Link) \
    (Tdf)->LowFragmentAddress = (_ULONG)((Link) | SONIC_END_OF_LIST)
...
{ \
    _SONIC_PHYSICAL_ADDRESS _Adr = (Adr); \
    PSONIC_RECEIVE_RESOURCE _Rrp = (Rrp); \
    (_Rrp)->LowBufferAddress = (_ULONG)(_Adr); \
    (_Rrp)->HighBufferAddress = (_ULONG)(SONIC_GET_HIGH_PART_ADDRESS(_Adr)); \
}
...
#define SONIC_GET_RECEIVE_RESOURCE_ADDRESS(Rrp) \
    (SONIC_PHYSICAL_ADDRESS)(Rrp)->LowBufferAddress
...
{ \
    _ULONG _Len = (Len); \
    PSONIC_RECEIVE_RESOURCE _Rrp = (Rrp); \
    (_Rrp)->LowBufferWordCount = (_ULONG)(((_Len) & 0x1ffff) >> 1); \
    (_Rrp)->HighBufferWordCount = (_ULONG)((_Len) >> 17); \
}
