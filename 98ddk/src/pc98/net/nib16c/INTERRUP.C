...

{
    ...
    if (LocalIsrValue == 0xFFFF)
    {  
      // IGNORE INTERRUPT DURING CARD ISORATION
      LocalIsrValue = 0;
    }
        ...
        if (Isr & SONIC_INT_LOAD_CAM_DONE) {
            ...
            if (Adapter->ResetInProgress) {
                ...
                case NdisRequestClose:
                    ...
                    --Open->References;
                    Adapter->SonicCloseWaitFlag = 0;  //INS-NEC
...

{
    ...
    if (Adapter->SonicCloseWaitFlag == 1) {
        // If DeferredProcess doesn't yet, waiting.
	NdisSetTimer(&Adapter->DeferredTimer,1000); 
        return;
    }
...

{
    ...
    do {
        ...
        SonicMemoryBankset( Adapter,
                NdisGetPhysicalAddressLow(Adapter->ReceiveDescriptorAreaPhysical));
        PacketAddress = (USHORT)CurrentDescriptor->LowPacketAddress;
        PacketPhysical = PacketAddress;
        PacketAddress = (USHORT)CurrentDescriptor->HighPacketAddress;
        ...
        SonicMemoryBankset( Adapter, 
                NdisGetPhysicalAddressLow(Adapter->ReceiveDescriptorAreaPhysical));
        ReceiveStatus = (USHORT)(CurrentDescriptor->ReceiveStatus);
        ByteCount = (USHORT)(CurrentDescriptor->ByteCount);
        ...
        SonicMemoryBankset( Adapter, PacketPhysical);
        SONIC_MOVE_MEMORY(
                &Adapter->LookAHeadData,                
                PacketVa,                               
                LookAheadSize
                );
        ...
        if (PacketSize < 14) {
            ...
            if (PacketSize >= ETH_LENGTH_OF_ADDRESS) {
                ...
                EthFilterIndicateReceive(
                    Adapter->FilterDB,
                    (NDIS_HANDLE)((PUCHAR)PacketPhysical + 14),// context
                    (PVOID)&Adapter->LookAHeadData,        // destination address       
                    (PVOID)&Adapter->LookAHeadData,        // lookahead buffer        
            ...
            EthFilterIndicateReceive(
                ...
                (PVOID)&Adapter->LookAHeadData,                // destination address    
                (PVOID)&Adapter->LookAHeadData,                // lookahead buffer        
                (UINT)14,                                // header buffer size
                (PUCHAR)&Adapter->LookAHeadData + 14,        // lookahead buffer       
                (UINT)LookAheadSize -14,                // lookahead buffer size
                (UINT)PacketSize - 14                        // packet size
                );
        }
...
SkipIndication:;
        ...
        CurrentDescriptor->InUse = SONIC_OWNED_BY_SONIC;
...

{
    ...
    TransmitStatus = (USHORT)TransmitDescriptor->TransmitStatus;
...
--*/
{
    ...
    if (Adapter->WakeUpTimeout) {
        ...
        if (!(TransmitStatus & SONIC_TCR_STATUS_MASK)) {
            ...
            TransmitDescriptor->TransmitStatus = SONIC_TCR_PACKET_TRANSMITTED_OK;
