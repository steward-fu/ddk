...
IFDEF	NEC_98
COMxPort	label	word
		dw	030h, 032h,032h,032h,032h,030h,030h,075h,077h,035h,033h
SIZE_OF_COMX	equ	$ - COMxPort
		dw	0b0h, 0B3h,0B3h,0B3h,0B3h,0B1h,0B1h,000h,000h,0B0h,0B0h
		dw	0b2h, 0BBh,0BBh,0BBh,0BBh,0B9h,0B9h,000h,000h,0B2h,0B2h
ENDIF	; NEC_98
;***
;
...
IFDEF	NEC_98
	pushf
	mov	ax, cx			; save port address.
	mov	esi,OFFSET32 COMxPort
	mov	ecx,MAXCOM + 1
@@:
	cmp	[esi], ax		; check Base port PnP
	je	@f			; Yes  - we found.
	add	esi,SIZE_OF_COMX	; search next
	loop	@b
	popf
	jmp	IF_Done			; in case of illigal.
@@:
	lea	edi,[ebx.BasePortPnP]
	mov	ecx,SIZE_OF_COMX / 2
	cld
	rep	movsw
	popf
ENDIF	; NEC_98
