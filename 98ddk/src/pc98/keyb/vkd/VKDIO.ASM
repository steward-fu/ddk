...
EXTRN	VKD_Clear_BIOS_Key_State:NEAR
EXTRN	VKD_Phys_SetRepRate:NEAR
EXTRN	VKD_Phys_SetLEDs:NEAR

EXTRN	VKD_VM_Server:NEAR
EXTRN	VKD_Set_Shift_State:NEAR
endif	;NEC_98
;******************************************************************************
...
ifdef	NEC_98
	in	al, pstat_Kybd
	and	al, 0C5h                        ;ErrorFlag, RDY-Clear
	test	[esi._8042_status], fKBS_DAV    ;Is there TrapDATA?
	jz	short cmdr_exit                 ;No.
	or	al,02h                          ;Yes,Set RDY when TrapDATA.
else	;NEC_98
...
ifdef	NEC_98
	...
	push	ebx				;Reset VKD's info.
	push	edx
	mov	dl, [esi.out_head]		;"head=tail" means "queue is empty"
	mov	[esi.out_tail], dl
	test	[esi._8042_status], fKBS_DAV	;Is there TrapDATA?
	jz	short skip_clear_int
	push	eax
	mov	eax, [VKD_irq_Handle]
	VxDCALL	VPICD_Clear_Int_Request
	and	[esi.kbdState], NOT KBS_sim_alt_hld
	pop	eax
skip_clear_int:
	and	[esi._8042_status], (not fKBS_DAV)
	mov	bx, [esi.loc_shift_state]
	and	bl, 1fh
	mov	[esi.loc_bios_key_state][14], bl
	pop	edx
	pop	ebx

	push	eax
	push	ebx
	mov	bl, [esi.loc_bios_key_state][14]
	and	bx, 1fh
	mov	ax, 0
	call	Update_Shift_State
	call	VKD_Clear_BIOS_Key_State

	pop	ebx			;When Win95 boot,Controll Caps/Kana LED
	push	ebx
...
BeginProc VKD_Trap_DataR, PUBLIC, High_Freq
	...
	add	esi, [VKD_CB_Offset]	; the VM's control block
ifdef	NEC_98
	Dispatch_Byte_IO Fall_Through, <VKD_Out_60>
else	;NEC_98
...
ifdef	NEC_98
	...
	shr	ebx, 3
	and	bl, 0fh
	mov	cl, [esi+ebx.loc_bios_key_state]	; Get VM Key State
	test	al, 80h
	jz	@f
	not	ch
	and	cl, ch
...
@@:
	or	cl, ch
	cmp	al, 71h				; Q: CAPS ON ?
	je	send_data			;    Y
	cmp	al, 0F1h			; Q: CAPS OFF ?
	je	send_data			;    Y
	cmp	al, 72h				; Q: KANA ON ?
	je	send_data			;    Y
	cmp	al, 0F2h			; Q: KANA OFF ?
	jne	no_caps_kana			;    N
send_data:
	push	ebx
	mov	bl, cl
	...
	pop	ebx
no_caps_kana:
	mov	[esi+ebx.loc_bios_key_state], cl	; Set VM Key State
	pop	ebx
	pop	ecx

if 0
	pushad						; Reflect the state to
	mov	eax, esi				; the LED right now
	sub	eax, [VKD_CB_Offset]			;
	cmp	eax, [VKD_Kbd_Owner]			; Q: Keyboard owner
	jnz	short @f				;    N: Skip

	mov	bl, [esi.loc_bios_key_state][14]	; Get local shift state
	mov	bh, byte ptr [vkd_gbl_shift_state]	; Get global shift state
	xor	bh, bl					;
	and	bh, 0110b				; Q: KANA/CAPS change ?
	jz	short @f				;    N: Same, skip

	xor	byte ptr [vkd_gbl_shift_state], bh	; Set global shift state
	or	bl, [edx.VKD_Numshift]			; get num shift
	xor	bh, bh					;
	call	VKD_Phys_SetLEDs			; Set LED
@@:	popad						;
endif
endif	;NEC_98
...
ifdef	NEC_98
;*************************************
	cmp	[esi.vkd_kbdcmd], 09dh			; Set LED 2nd Command ?
	jnz	not_led_set				;  N:
	mov	[esi.vkd_kbdcmd], 0			; Set LED Command Done !

	; Set LED command (2nd Command)
	; al = 0111xxxxb
	;          ^^^^
	;          ||||
	;          |||+-NUM    0:OFF  1:ON
	...
	;                   +-- NumLock     0:OFF  1:ON
	;                   |
	; vkd_numshift    = x0000000b
	; loc_shift_state = 000xxxxxb
	; gbl_shift_state = 000xxxxxb
	;                      |||||
	;                      ||||+-SHIFT  0:OFF  1:ON
	;                      |||+--CAPS   0:OFF  1:ON
	;                      ||+---kana   0:OFF  1:ON
	...
	mov	ah, al
	shl	al, 7
	mov	[esi.vkd_numshift], al
	shr	ah, 1
	and	ah, 6
	mov	al, not 6
	and	byte ptr [esi.loc_bios_key_state][14], al
	or	byte ptr [esi.loc_bios_key_state][14], ah
	and	byte ptr [esi.loc_shift_state], al
	or	byte ptr [esi.loc_shift_state], ah
%out	Ownner Check ?
if 0
	cmp	ebx, [VKD_Kbd_Owner]
	jz	short @f
...
@@:	and	byte ptr [vkd_gbl_shift_state], al
	or	byte ptr [vkd_gbl_shift_state], ah
	pop	eax
	ror	al, 1
	mov	bl, al
	call    VKD_Phys_SetLEDs
	jmp	short data_exit
not_led_set:
	cmp	[esi.vkd_kbdcmd], 09ch			; Set AutoRepeat 2nd Command ?
	jnz	not_rep_set				;  N:
	mov	[esi.vkd_kbdcmd], 0			; Set AutoRepeat Command Done !
	mov	[esi.vkd_repcmd], al
%out	Ownner Check ?
if 0
	cmp	ebx, [VKD_Kbd_Owner]
	jnz	short data_exit
...
not_rep_set:
ifdef	NEC_WINKEY
	cmp	[esi.vkd_kbdcmd], 095h			; Set ModeSet 2nd Command ?
	jnz	not_mode_set				;  N:
	mov	[esi.vkd_kbdcmd], 0			; Set ModeSet Command Done !
	mov	[esi.vkd_modecmd], al
	jmp	data_exit
not_mode_set:
endif	;;NEC_WINKEY
	cmp	al, 09dh			; Set LED Command (1st Command)
	jne	not_led_cmd			;  N:
	mov	[esi.vkd_kbdcmd], al		;  Y: Set Flag
ifdef	NEC_WINKEY
	jmp	data_exit
else	;;NEC_WINKEY
	jmp	short data_exit
...
not_led_cmd:
	cmp	al, 09ch			; Set AutoRepeat Command (1st Command)
	jne	not_rep_cmd			;  N:
	mov	[esi.vkd_kbdcmd], al		;  Y: Set Flag
not_rep_cmd:
ifdef	NEC_WINKEY
	cmp	al, 095h			; Set ModeSet Command (1st Command)
	jne	not_mode_cmd			;  N:
	mov	[esi.vkd_kbdcmd], al		;  Y: Set Flag
not_mode_cmd:
endif	;;NEC_WINKEY
...
ENDIF
	...
	jne	send_ack		;   N:
	VxDcall	VKD_Set_LEDs		;   Y: set LED's with value in al
	jmp	send_ack
...
not_led_cmd:				; must have been special "set repeat
	...
	test	al, 80h 		;Q: new command byte?
	jnz	abort_cmd		;   Y:
	VxDcall	VKD_Set_Key_Rate	; set rates with values in al
