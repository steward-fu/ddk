...
ifdef	NEC_98
VPICD_IRQ_Descriptor <1,,OFFSET32 VKD_Int_Handler,OFFSET32 VKD_Virt_Int,\
...
ifdef	NEC_98
VKD_Inst1 InstDataStruc <,,502h, 32, ALWAYS_Field>  ; Keycode Buf
VKD_Inst2 InstDataStruc <,,524h, 2 , ALWAYS_Field>  ; head address of Keycode area
VKD_Inst3 InstDataStruc <,,526h, 2 , ALWAYS_Field>  ; end address of Keycode area
VKD_Inst4 InstDataStruc <,,528h, 1 , ALWAYS_Field>  ; key count of Keycode area
VKD_Inst5 InstDataStruc <,,52Ah, 16, ALWAYS_Field>  ; Press key status(96bit)
VKD_Inst6 InstDataStruc <,,53Ah, 1,  ALWAYS_Field>  ; keys shift status
VKD_Inst7 InstDataStruc <,,522h, 2,  ALWAYS_Field>  ; Key SHIFT TABLE
;; These declarations is for High-res.
VKD_InstH1 InstDataStruc <,,408h, 8,  ALWAYS_Field>  ; KeyCode for SHIFT Key
VKD_InstH2 InstDataStruc <,,410h, 4,  ALWAYS_Field>  ; Buffer start address
VKD_InstH3 InstDataStruc <,,414h, 4,  ALWAYS_Field>  ; KB_ENTRY_TBL
VKD_InstH4 InstDataStruc <,,418h, 4,  ALWAYS_Field>  ; internal interrupt Table
else	;NEC_98
...
EXTRN   VKD_Last_LED:BYTE
EXTRN   VKD_Last_Key_Rate:BYTE
...
ifdef	NEC_98
EXTRN	Kbd_Debug_Trap_Code:BYTE

ifdef	NEC_SHIFT
EXTRN	Keyboard_Shift_Section:BYTE
EXTRN	Keyboard_Shift_Entry:BYTE
EXTRN	VKD_Phys_KeyState:DWORD
endif	;NEC_SHIFT
endif	;NEC_98
...
;
;Set a kind of keyboard
;     xxxxxxxx
;     |||    + 0:N-mode,              1:H-mdoe
;     ||+----+ 0:Not-NumLock          1:NumLock
;     |+-----+ 0:HardwareLockKeyboard 1:SoftwareLockKeyboard
;     +------+ 0:PC-9800 Series       1:PC-H98 Series
;
public	VKD_MachineID
VKD_MachineID	    db  0		; 
endif	;NEC_98
...
ifdef	NEC_98
PUBLIC	gbl_intdata
gbl_intdata		db	0	;Newest ReadData when Hardware Interrupt 9.

PUBLIC	lederr
lederr			db	0ffh	;WrongData When getting LED reponse.

PUBLIC  physaddr_cmos			;Rewritting CMOS is rewritting Phys.
physaddr_cmos           dd      ?       ;CMOS PhysPages for rewritting

grph_ret_found		db	0	;0:set_focus 1:graph+ret

; Counter for ResponseCheck of SET_LED Routine
LED_COUNTER		DB	0
loc_shift_save		dw	0

PUBLIC	Note_Cmd_17			; KBD CTRL is D78011BGC-635
Note_Cmd_17		db	0	;          or D78011BGC-691
endif	;NEC_98
...
ifdef	NEC_98
VKD_ignore_key	    dd	0ffh		; virtual key that should be ignored
else	;NEC_98
...
ifdef	NEC_98
PUBLIC	VKD_Trap_Code
VKD_Trap_Code       db	0ffh

ifdef	NEC_SHIFT
PUBLIC	VKD_Shift_Separate
VKD_Shift_Separate  db	0
PUBLIC	VKD_Shift_onoff
VKD_Shift_onoff     db  0
endif	;NEC_SHIFT
endif	;NEC_98
	ALIGN 4
...
ifdef	NEC_98
PUBLIC	VKD_RepDelay
VKD_RepDelay		dw	500		; S/W key repeat delay
PUBLIC	VKD_RepRate
VKD_RepRate		dw	63		; S/W key repeat rate
PUBLIC	VKD_RepMode
VKD_RepMode		dw	3		; Repeat mode

;						from kbd.drv (AT)
;			ms	scale on cpl	rate	chr/sec
VKD_Rate_Table	    dw	500	;0		31	2.0
		    dw	500	;1		31	2.0
		    dw	500	;2		31	2.0
		    dw	333	;3		26	3.0
		    dw	250	;4		23	4.0
...
VKD_Delay_Table	    dw	250, 500, 750, 1000

;repeat table. copy from keyboard.drv (setspeed.asm rate_table)
rep_rate_table  db      31              ;0
                db      31              ;1
                db      31              ;2
                db      26              ;3
                db      23              ;4
...
ifdef	NEC_98
DefineState	SC_LShf,  SS_Shift
DefineState	SC_CpLok, SS_Caps
DefineState	SC_KanaLok, SS_Kana
DefineState	SC_Alt,   SS_Alt
DefineState	SC_Ctrl,  SS_Ctrl
ifdef	NEC_SHIFT
...
gSpecial106 db   0              ; special 106 keyboard indicator.
endif ;NEC_98
endif

state_table	label byte	; for conversion from state to scan codes
ifdef	NEC_98
    State_Conv_Struc <SC_LShf,	 SingleCase> ; 0     SHIFT
    State_Conv_Struc <SC_CPLOK,  SingleCase> ; 1     CAPS
    State_Conv_Struc <SC_KANALok,SingleCase> ; 2     kana
    State_Conv_Struc <SC_Alt,	 AltCase>    ; 3     GRPH
    State_Conv_Struc <SC_Ctrl,	 SingleCase> ; 4     CTRL
else	;NEC_98
    State_Conv_Struc <,>		    ; 0
...
ifdef	NEC_98
EXTRN	VKD_Phys_Initialize:NEAR
EXTRN	VKD_Phys_SetRepRate:NEAR
EXTRN	VKD_Phys_SetLEDs:NEAR
EXTRN	VKD_Phys_GetLEDs:NEAR
EXTRN	VKD_NOTE_Menu:NEAR
endif	;NEC_98
...
ifdef	NEC_98
	test	edx, 080000000h	; Q: KBD CTL D78011BGC-635 or D78011BGC-691 ?
	jz	@f		;   N:
	and	edx, 7FFFFFFFh	;
	mov	[Note_Cmd_17], 01h	;
@@:
	mov	[nul_device_addr], edx
endif	;NEC_98
;
...
;
	...
	VMMCall Get_Profile_Decimal_Int
        ...
        ; subtype 0 or 1 don't need to patch state_table.
	cmp	eax, 0                     ; Q: is this 101 keyboad?
	jz	short state_table_patched  ;   Y: do nothing
	cmp	eax, 1                     ; Q: is this AX keyboard?
	jz	short state_table_patched  ;   Y: do nothing
        cmp     eax, 5                     ; Q: OEM keyboards?
        jae     short state_table_patched  ;   Y: do nothing.

        ; To patch handlecase for CapsLock with ToggleWithShiftCase.
	mov	[state_table+(6*2).handlerCase], ToggleWithShiftCase
        mov     gSpecial106, 1             ; special 106 keyboard indicator.
...
ifdef	NEC_98
	...
	mov	eax, 0ffh		; Set default
	mov	edi, OFFSET32 Kbd_Debug_Trap_Code
	VMMCall Get_Profile_Hex_Int
	mov	[VKD_Trap_Code], al	; Save scan code

ifdef	NEC_SHIFT

	mov	eax, 1
	mov	esi, offset32 Keyboard_Shift_Section
	mov	edi, offset32 Keyboard_Shift_Entry
	VMMCall	Get_Profile_Boolean
	cmp	al,0			; shiftspa = off ?
	jnz	@f			; no
	mov	[VKD_Shift_Separate], 1	; yes, shift separate off
...
ifdef	NEC_98
	...
	push	ebx
	xor	al, al
	mov	ebx, BIOS_NH_select
	test	byte ptr [ebx], 00001000b	;Q:N mode?
	jz	short not_Hmode			; Yes.
	or	al, 1
not_Hmode:
	mov	ebx, BIOS_NPC_select
	test	byte ptr [ebx], 10000000b	;Q:PC-H98 Series?
	jz	short not_npc			; No.
	or	al, 80h
not_npc:
	mov	ebx, BIOS_KeyBoard_select
	test	byte ptr [ebx], 01001000b	;Q:SoftwareLocked Key
	jz	short ID_end			; No.
	or	al, 40h
	mov	ebx, BIOS_KeyBoard_select
	test	byte ptr [ebx], 00001000b	;Q:Laptop?
	jz	short ID_end			; No.
	or	al, 20h
ID_end:
	mov	[VKD_MachineID], al
	pop	ebx
	pop	eax
else	;NEC_98
;
...
ifdef	NEC_98
;
; hook software INT 6fh (ATOK6/7)
;
	mov	eax, 6fh                    ; Hook INT 6fh
	mov	esi, OFFSET32 vkd_int_6f
...
ifdef	NEC_98
	push	eax
	VMMCall	_MapPhysToLinear,<0A3000h,10000h,0>
	mov	[physaddr_cmos],eax
	pop	eax
endif	;NEC_98
...
ifdef	NEC_98
	...
	or	eax,eax
	jz	VKD_Init_Fatal
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_Inst6>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_Inst7>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
	test	[VKD_MachineID],1                               ;Q:H-mode?
	jz	short instance_end                              ; No:Skip H-mode instanced
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_InstH1>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_InstH2>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_InstH3>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
	VMMCall	_AddInstanceItem, <<OFFSET32 VKD_InstH4>, 0>
	or	eax,eax
	jz	VKD_Init_Fatal
instance_end:
else	;NEC_98
	...
	; Check if VPOWERD.VXD is loaded...
	VxDcall	_VPOWERD_Get_Version
	jc	@F

	; Register with VPOWERD's Power Handler
	mov	esi,OFFSET32 VKD_Power_Handler
	VxDcall	_VPOWERD_Register_Power_Handler, <esi,0>
IFDEF 	DEBUG
	or	eax,eax
	TRAPnz				  
ENDIF
...
ifdef	NEC_98
	...
	VMMcall Exec_Int
	cmp	[ebp.Client_BH], 0
	je	short buf_empty 	    ;	N: we're done!
...
;
	...
	push	ebx
ifdef	NEC_98
	mov	[esi.vkd_repcmd], 51h
ifdef	NEC_WINKEY
	mov	[esi.vkd_modecmd], 03h		; Win95 mode
endif	;;NEC_WINKEY
	mov	[esi.vkd_paste_flag], 0
	mov	[esi.vkd_paste_mode], PASTE_MODE_INT9
	mov	[esi.dos_timeout], 0
	mov	[esi.vkd_retry], 0
	call	VKD_Phys_Initialize
endif	;NEC_98
	mov	ebx, [gbl_shift_state]
ifdef	NEC_98
	and	ebx, SS_Lock_mask	    ; don't modify lock key state
else	;NEC_98
...
;
	BEGIN_Touch_1st_Meg
ifdef	NEC_98
	movzx	eax, byte ptr ds:[BIOS_Shift_State_Loc] ; read last shift state
else	;NEC_98
...
ifdef	NEC_WINKEY
	mov	ebx, [Sys_VM_Handle]
	mov	eax, ebx
	add	eax, [VKD_CB_Offset]	    ; the VM's control block
	mov	[eax.vkd_modecmd], 01h	    ; Set as normal mode
endif	;;NEC_WINKEY

ifdef	NEC_98
	call	VKD_VM_Init
	mov	ecx, 50000			; 50ms loop
@@:	out	5Fh, al				;
	loop	short @b			;
	clc
	ret
...
ifdef	NEC_98
	mov	[ebx.vkd_repcmd], 51h
ifdef	NEC_WINKEY
	mov	[ebx.vkd_modecmd], 03h		; Win key should be activated
						; even in DOS BOX as same as
						; US/FE.
						; So we don't have to set to
						; normal mode
endif	;;NEC_WINKEY
	mov	[ebx.vkd_paste_flag], 0
	mov	[ebx.vkd_paste_mode], PASTE_MODE_INT9
	mov	[ebx.dos_timeout], 0
endif	;NEC_98
...
ifdef	NEC_98
	...
	mov	ax, 3fh 		    ; HELP
	ShiftState <SS_Caps + SS_Kana>, <SS_Ctrl + SS_Alt>
	mov	cl, CallOnPress
	mov	esi, OFFSET32 VKD_NOTE_Menu
	or	edx, -1
	xor	edi, edi		    ; No priority time out
	VxDCall VKD_Define_Hot_Key
...
ifdef	NEC_98
;This problem exist when Windows 3.0.
;When Change VM, VKD need to stack BreakCode into VM's keyBuffers for Current VM
;as Getting Only MakeCode.
	...
	xor	edx, edx		; Flag for Use or not use Queue_Output
					; Default=0
...
VSF_ByteLoop:
	...
	mov	ah, 1			;Test Bit
VSF_BitLoop:
	cmp	bl, SC_KANALok			; Do nothing for lock key
	je	short Not_MakeBit
	cmp	bl, SC_CpLok
	je	short Not_MakeBit
	push	ebx					;@
	movzx	ebx, bh					;@
	test	[esi.loc_bios_key_state+ebx], ah	;Last ScanCode is MakeCode?
	pop	ebx					;@
	jz	Not_MakeBit				;@ No..
	push	ebx
	movzx	ebx, bh
	xor	[esi.loc_bios_key_state+ebx], ah	;clear make bit
	pop	ebx
	mov	al,bl					;Copy ScanCode to AL
	or	al, 80h					;Create BreakCode
	call	Queue_Output				;Stack VM's keyBuffers
	inc	edx					;inc Flag

Not_MakeBit:
	inc	bl			;Total Counter(ScanCode)
	shl	ah, 1
	loop	VSF_BitLoop
	...
	and	[esi.loc_shift_state], (SS_Kana + SS_Caps); Clear shift state
							; except lock key

	or	edx, edx				; Queue_Output used?
	jz	VSF_Not_Keyboard_Int			;  No
	mov	edx, esi				; VM_Handle + CB_Offset
	call	VKD_VM_Server				; Simulate KBD int

VSF_Not_Keyboard_Int:
	pop	edx
	pop	esi
	pop	ecx
	pop	ebx
...
;
; When New VM(ebx) is created,
; Old VM(VKD_Kbd_Owner) is dettached and the newest VM(ebx) is attached.
; When so if the machine is PC-H98 Series, NUM_LED status must be saved.
;
	push	eax
	push	ebx
	call	VKD_Phys_GetLEDs
	jc	short @f
	and	al,01h
	shl	al,7
	mov	ebx,[VKD_Kbd_Owner]
	add	ebx,[VKD_CB_Offset]
	mov	[ebx.vkd_numshift],al
@@:	pop	ebx
	pop	eax
;This code is for BUGFIX, MessageBox title Bar is changed ,when pressing GRPH+RET(or SPACE).
;By Shift Magement ,when SysVM gets graphBreakÅ®Focus,
;it returns graphMake&graphBreak to SysVM.
	push	esi
	cmp	[grph_ret_found],1
	jne	short not_grph_emulate
	mov	[grph_ret_found],0
	cmp	ebx,[VKD_kbd_Owner]
	je	short not_grph_emulate
	VMMcall Test_Sys_VM_Handle	    ;Q: Is Attach VM system VM?
	jne	short not_grph_emulate      ;No.  normal procese(jmp)
graph_make_chk:
	mov	esi,ebx
	add	esi,[VKD_CB_Offset]
	test	[esi.loc_shift_state],00001000b
	jnz	short not_grph_emulate
	push	eax
	mov	al,SC_ALT
	call	Queue_Output
	mov	al,0f3h
...
ifdef	NEC_98				    ; 
	jnz	short @f		    ; possible to do key repeat
	mov	al, 51h			    ; on DOS VM
	call	VKD_Phys_SetRepRate	    ; set default value
	jmp	short focus_exit
@@:
else	;NEC_98
...
ifdef	NEC_98
	mov	al, [edx.vkd_repcmd]
	call	VKD_Phys_SetRepRate
endif	;NEC_98
...
;   VKD_Power_Handler
;
;   DESCRIPTION:    Look for power restoration and reset the keyboard
;
;   ENTRY:	    
;
;   EXIT:
...
hkd_00:
	...
	Trace_OutNC "VKD_Define_HKEY Pre-empted"
...
ifdef	NEC_98
	...
	cmp	ecx, 0ff00h
	pop	ecx
	jne	short not_change
	push	eax
	and	ecx, not ( 0ff00h + SS_Caps + SS_Kana )
	and	eax, ( SS_Caps + SS_Kana )
	or	ecx, eax
	pop	eax
	mov	ebx, ecx
not_change:
...
ifdef	NEC_SHIFT
	...
	jz	short shift_key
	cmp	al, 0FDh
	jnz	short no_shift_process
shift_key:
	cmp	[VKD_Shift_Separate], 1		;Q: Shift sepa On?
	jz	short shift_sepa_off3		;   N:
	push	ebx
	mov	ebx, [VKD_kbd_Owner]
	VMMCall	Test_Sys_VM_Handle		;Q: DOS prt?
	pop	ebx				;
	jz	short no_shift_process		;   N:
shift_sepa_off3:
	test	al, 0Dh				;Q: R-Shift?
	jz	short l_shift			;   N:L-Shift
	test	al, 080h			;Q: R-Shift Break?
	jnz	short r_shift_break		;   Y:
	or	[VKD_Shift_onoff], 01h		;   Set R-Shift bit
@@:
	test	[VKD_Shift_onoff], 02h		;Q: L-Shift On?
	jz	short no_shift_process		;   N:R-Shift only
	dec	cx				;   Y:
	jz	force_exit			;   cx = 0
	jmp	fk_next				;   cx != 0
r_shift_break:
	and	[VKD_Shift_onoff], not 01h	;   Clear R-Shift bit	
	jmp	@b


l_shift:
	test	al, 080h			;Q: L-Shift Break?
	jnz	short l_shift_break		;   Y:
	or	[VKD_Shift_onoff], 02h		;   Set L-Shift bit
@@:
	test	[VKD_Shift_onoff], 01h		;Q: R-Shift On?
	jz	short no_shift_process		;   N:L-Shift only
	dec	cx				;   Y:
	jz	force_exit			;   cx = 0
	jmp	fk_next				;   cx != 0
l_shift_break:
	and	[VKD_Shift_onoff], not 02h	;   Clear L-Shift bit
	jmp	@b
no_shift_process:
endif	;NEC_SHIFT
endif	;NEC_98
...
loop_copy_paste_data:
.errnz Paste_ShiftState - 2
	lodsd
	stosw
	xor	ax, ax
	bt	eax, 16+15
...
ifdef	NEC_98
	mov     [edi.linear_strategy], 0
	mov     [edi.linear_intentry], 0

	call    set_paste_dos                   ; dos paste
	jnc     short sp_exit
...
ifdef	NEC_98
	and	eax, 0000ffffh		    ;Actually, uses ax
endif
...
ifdef	NEC_98
	dd	OFFSET32 VKD_API_Set_Speed	; [2]
endif	;NEC_98
...
ifdef	NEC_98
	and	edx, SS_Toggle_mask OR SS_Toggle_Dn_mask OR SS_Shift_mask OR SS_Lock_mask
else	;NEC_98
...
;   VKD_API_Set_Speed
;
;   DESCRIPTION:    Set Speed for S/W key repeat
;
;   ENTRY:	[Client_ECX]
;		
;		cl : Repeat Rate
...
;==============================================================================
EndDoc
BeginProc VKD_API_Set_Speed, DOSVM, PUBLIC

	pushad

	mov	ebx, [ebp.Client_EBX]
	mov	VKD_RepMode, bx			; Set RepMode

	movzx	ebx, [ebp.Client_CL]
	mov	ax, [VKD_Rate_Table][ebx*2]
	mov	VKD_RepRate, ax			; Set RepRate

	movzx	ebx, [ebp.Client_CH]
	mov	ax, [VKD_Delay_Table][ebx*2]
	mov	VKD_RepDelay, ax		; Set RepDelay

	movzx   ebx, [ebp.Client_CL]
	mov 	al, byte ptr [rep_rate_table][ebx]

	mov	bl, [ebp.Client_CH]		; Delay value
	shl	bl, 5
	or	al, bl                		; Rate	value
        VxDCall VKD_Set_Key_Rate
...
ifdef	NEC_98
	cmp     [esi.vkd_paste_flag], 0         ; is nesting
	jnz     short server_not_pasting
	cmp     [esi.vkd_paste_mode], PASTE_MODE_DOS
	jz      short server_not_pasting
endif	;NEC_98
...
ifdef	NEC_98
	mov	[VKD_ignore_key], 0ffh	    ;	   don't need to ignore anymore
else	;NEC_98
...
ifdef	NEC_98
	jz	short @f				;   N: queue key
	push	eax					;   Y:
	mov	ax, [loc_shift_save]
	mov	[esi.loc_shift_state], ax
	pop	eax
	jmp	VKD_Beep
@@:
...
chk_msg_mode:
    ;
    ;  If (and only if) we are in message mode,
    ;  get the next key and queue it as a message mode key.
    ;
	TestMem [esi.kbdState], KBS_msg_mode ;Q: special message mode?
ifdef	NEC_98
	jz	server_exit		    ;	N:
...
; setup for updating the VM's shift state
	...
	mov	ebx, [gbl_shift_state]
ifdef	NEC_98
	and	ebx, SS_Lock_mask	    ; clear all "down" bits
else	;NEC_98
...
ifdef	NEC_98
	push	esi
	sub	esi,[VKD_CB_Offset]
	xchg	esi,ebx
	VMMcall	Test_Sys_VM_Handle		;Q: system VM?
	xchg	esi,ebx
	je	short not_graph_ret		;Yes.graph+RET(Space)
	cmp	al,SC_Enter
	je	short Enter_Space_found
	cmp	al,34h				;34h=Space key
	jne	short not_graph_ret
Enter_Space_found:
	mov	[grph_ret_found],1
not_graph_ret:
	pop	esi
endif	;NEC_98
...
qp_exit:
ifdef	NEC_98
	dec	[esi.vkd_paste_flag]
endif	;NEC_98
	ret

qp_paste_char_avail:
ifdef	NEC_98
	test	[esi.atok_warning], ATOK_WARNING_REST or ATOK_WARNING_CODE or ATOK_WARNING_OFF
	jz	short qp_paste_not_atok_warning
	xor	eax, eax		; ESC
	mov	ebx, eax		; no shift
	test	[esi.atok_warning], ATOK_WARNING_CODE
	jnz	short @f
	mov	eax, 1ch		; RETURN
	test	[esi.atok_warning], ATOK_WARNING_REST
	jnz	short @f
	mov	ebx, 10h		; CTRL
	mov	eax, 35h		; XFER
@@:
	...
	pop	eax
	Queue_Stroke
	and	[esi.atok_warning], not (ATOK_WARNING_REST or ATOK_WARNING_CODE or ATOK_WARNING_OFF)
	jmp	qp_wait_exit

qp_paste_not_atok_warning:
	mov	eax, edi
...
;
	...
	xor	ebx, ebx
	call	Update_Shift_State
	mov	al, [esi.atok_num_ank]		;
	call	change_mode_atok		;
	movzx	eax, [esi.loc_shift_state]
	pop	ebx
	call	Update_Shift_State
...
;
; Paste KANJI by code input in shift jis
;
qp_not_ank:
	xor	ebx, ebx
	call	Update_Shift_State
	mov	eax, [esi.paste_ptr]
	cmp	eax, [esi.paste_end]
	jz	qp_exit

	test	[esi.vkd_paste_mode], PASTE_MODE_ATOK6 or PASTE_MODE_ATOK7
	jz	qp_server
	mov	al, [esi.atok_num_code]		;
	call	change_mode_atok		;

	mov	ah, [edi.Paste_Char]
	cmp	ah, 81h
	jc	qp_server
	cmp	ah, 0a0h
	jc	short @f
	...
	jnc	qp_server
@@:
	mov	al, [edi.Paste_Char+(SIZE Paste_Rec)]
	cmp	al, 40h
	jc	qp_server
	cmp	al, 0fdh
	jnc	qp_server
	...
	add	edi, (SIZE Paste_Rec)*2	    ; update paste buffer ptr
	mov	[esi.paste_ptr], edi

	shl	eax, 16
	mov	ebx, eax
	shr	ebx, 28
	Queue_Stroke [ebx+Hex_SCs]
	mov	ebx, eax
	shr	ebx, 24
	and	ebx, 0fh
	Queue_Stroke [ebx+Hex_SCs]
	...
	and	ebx, 0fh
	Queue_Stroke [ebx+Hex_SCs]
	Queue_Stroke 1ch			; RETURN
	mov	al, [esi.atok_num_def]
...
qp_wait_exit:
	...
	pop	edx
	pop	eax
	or	[esi.kbdState], KBS_queue_hold
	jmp	qp_exit
...
ifdef	NEC_98
	and	eax, 0ffffh		; mask out high word
	and	ebx, 0ffffh
endif
...
ifdef	NEC_98
	mov	ecx, (NOT (SS_Lock_Mask)) AND 0FFFFh
else
...
ifdef	NEC_98
	and	ecx, (SS_Caps + SS_Kana)	; isolate toggles
else
...
ifdef	NEC_98
	...
	push	ebx
	push	esi
	test	[edi.vkd_paste_mode], PASTE_MODE_DOS
	jz	short ep_not_dos
...
ep_not_dos:
	...
	test	[edi.vkd_paste_mode], PASTE_MODE_ATOK7
	jz	short ep_not_atok7
...
ep_not_atok7:
	...
	test	[edi.vkd_paste_mode], PASTE_MODE_ATOK6 or PASTE_MODE_ATOK7
	jz	short ep_not_atok

	push	ebx
	movzx	eax, [edi.loc_shift_state]
	movzx	ecx, [edi.atok_save]
	xchg	esi, edi
	or	ecx, ecx
	jnz	short ep_not_atok_off

	mov	ebx, 10h		; CTRL
	call	Update_Shift_State
	pop	ebx
	Queue_Stroke 35h		; XFER
	jmp	short ep_restore_atok
...
;It must keep status before a paste after a paste.
	cmp	ebx, [VKD_Kbd_Owner]		; Q: Four/Back
	je	short force_shift_change
	movzx	ebx, [edi.before_shift_paste]	; 1. BACK
	jmp	short change_to_ebx

force_shift_change:
	test	[VKD_MachineID],01000000b	; Q: SoftLock/HardLock
	jz	short change_to_global		; 3. FOUR-HARD

	movzx	ebx, [edi.loc_shift_state]	; 2. FOUR-SOFT
	and	ebx, not ( SS_Caps + SS_Kana )
	movzx	eax, [edi.before_shift_paste]
	and	ax, SS_Caps + SS_Kana
	or	ebx, eax				;Restore VM's shift(LED) state
	xchg	eax, ebx
	call	VKD_Phys_SetLEDs
	xchg	eax,ebx
	and	[vkd_gbl_shift_state], not ( SS_Caps + SS_Kana )
	or	[vkd_gbl_shift_state], eax
...
ep_exit:
ifdef	NEC_98
	mov	[edi.vkd_paste_mode], 0
	mov	[edi.dos_timeout], 0
endif	;NEC_98
	popad
	ret
...
ifndef	NEC_98
	...
	cmp	al, 0F7h		; Q: LWin Up ?
	je	Win_Key_Up
	cmp	al, 0F8h		; Q: RWin Up ?
	jne	Win_Key_Set_Reg
...
ifdef	NEC_98
	movzx	eax, byte ptr [ebx.BIOS_Shift_State_Loc]
else	;NEC_98
...
; get current state words
	...
	sub	al, shft_tab_min	;Q: below table?
ifdef JAPAN
        jb      ss_exit                 ;  Y: can't change shift state
else
...
ifdef JAPAN
        jz      ss_exit                 ;  N: no need to change state
else
...
no_prefix:
	...
	jz	short key_press
ifdef	NEC_98
ifdef	NEC_SHIFT			;Non press. Really?
	cmp	cl, SC_LShf+SC_Break		;L-Shift?
	jz	short shift_LR_Release	;yes		
	cmp	cl, SC_RShf+SC_Break		;R-Shift?
	jnz	short Non_shift_Press	;no
shift_LR_Release:
	pushad
	mov	eax, VKD_Phys_KeyState[14] ;Get L-R Shift State
					;al:xxxxxxxL  L= 1:On,0:Off
					;ah:xxRxxxxx  R= 1:On,0:Off
	and 	eax, 2001h		;Q: shift on?
	jnz	@f			;  Y:
	popad
	jmp	short Non_shift_Press
@@:	popad				;Shift on
	jmp	short update		;do not change Shift state bit
Non_shift_Press:	
endif	;NEC_SHIFT
endif	;NEC_98
...
ifndef	NEC_98
        ...
        test    ax, SS_CAPLOCK
        jz      @F
        test    bx, SS_LSHIFT+SS_RSHIFT
        jnz     @F
        and     al, NOT SS_CAPLOCK      ; actually not modifying...
        and     ah, NOT SS_CAPLOCK      ; actually not modifying...
@@:
endif	;NEC_98
endif
...
ifdef NEC_98	;;debug
        mov     al, [esi.loc_bios_key_state][14]
        Trace_Out "local key state of shifts: #al"

        mov     eax, [ebx.CB_High_Linear]
        add     eax, BIOS_Shift_State_Loc
        mov     al, [eax]
        Trace_Out "BIOS area shift state: #al"
endif
...
ifdef NEC_98	;;debug
        mov     al, ds:[BIOS_Shift_State_Loc]
        Trace_Out "Global BIOS area shift state: #al"
endif
...
;
;==============================================================================
BeginProc VKD_Real_Init

	mov	ah, 03h				; Keyboard reset
	int	18h				;
	mov	cx, 50000			; 50ms loop
@@:	out	5Fh, al				;
	loop	short @b			;
	...
	mov	cx, 100h
loop_find_nul:
	cmp	es:[bx], dword ptr ' LUN'
	jnz	short not_find_nul
	cmp	es:[bx+4], dword ptr '    '
	jz	short found_nul
not_find_nul:
	inc	bx
	dec	cx
	jnz	loop_find_nul
	jmp	short vkd_real_init_ret
...
found_nul:
	...
	sub	bx, 0ah
	movzx	eax, bx			; es:bx is 'NUL' device seg:off address
	mov	dx, es
	shl	edx, 4
	add	edx, eax		; edx is 'NUL' device linear address

loop_dos_con:
	mov	eax, es:[bx]
	cmp	eax, -1
	jz	short vkd_real_init_ret
	les	bx, es:[bx]

	cmp	es:[bx+10], dword ptr ' NOC'
	jnz	loop_dos_con
	cmp	es:[bx+14], dword ptr '    '
	jnz	loop_dos_con
					; es:bx is 'CON' device seg:off address
	mov	ax, es:[bx+6]		; strategy address(offset)
	mov	[vkd_inst1R], eax
	mov	ax, es:[bx+8]		; intentry address(offset)
	mov	[vkd_inst2R], eax
	mov	si, offset vkd_inst1R
vkd_real_init_ret:
	push 	ds
	mov	cx, 0F8E8h		; system common area in ROM
	mov	ds, cx			;
	cmp	word ptr ds:[0], 2198h	; Q: 9821 ?
	jne	no_9821			;   N:
;
;	Check KBD Controller D78011BGC-635
;
	mov	bx, 3Fh
	cmp	byte ptr ds:[bx], 04h		; Q: YJ21 ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 10h		; Q: YJ27T/D ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 11h		; Q: YJ28 ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 19h		; Q: Y88WD ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 1Bh		; Q: YJ23 ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 1Ch		; Q: Y70 ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 1Eh		; Q: Y88WT ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 1Fh		; Q: YJ70T/D ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 20h		; Q: YJ8H ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 27h		; Q: YL77T ?
	je	set_note_cmd			;   Y:
;
;	Check KBD Controller D78011BGC-691
;
	cmp	byte ptr ds:[bx], 26h		; Q: YJ55/55'KAI ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 2Ch		; Q: YJ71T/D ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 31h		; Q: YJ56 ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 36h		; Q: YL99T ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 4Ah		; Q: YL98S ?
	je	set_note_cmd			;   Y:
	cmp	byte ptr ds:[bx], 3Eh		; Q: YJ57S/V 3E?
	jne	no_9821				;   N:
set_note_cmd:
	or	edx,080000000h		; set note cmd bit
no_9821:
	pop	ds
