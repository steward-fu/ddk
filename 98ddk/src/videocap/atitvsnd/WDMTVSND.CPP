...
NTSTATUS CWDMTVAudio::AdapterCompleteInitialization( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTVAudio:AdapterCompleteInitialization() ntStatus=%x\n",
		ntStatus));
...
BOOL CWDMTVAudio::AdapterUnInitialize( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:AdapterUnInitialize()\n"));
...
BOOL CWDMTVAudio::AdapterGetStreamInfo( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:AdapterGetStreamInfo()\n"));
...
BOOL CWDMTVAudio::AdapterQueryUnload( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:AdapterQueryUnload()\n"));
 ...
 * Author	: IKLEBANOV
 *^^*/
PVOID CWDMTVAudio::operator new( UINT size_t,  PVOID pAllocation)
{

	if( size_t != sizeof( CWDMTVAudio))
	{
		_DebugPrint(( DebugLevelVerbose, "CWDMTVAudio: operator new() fails\n"));
		return( NULL);
	}
	else
		return( pAllocation);
	...
	:m_CATIConfiguration( pDeviceObject, pCScript, puiErrorCode)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:CWDMTVAudio() enter\n"));
	...
	ENSURE
	{
		 ...
		 * puiErrorCode = WDMMINI_NOERROR;
		 _DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:CWDMTVAudio() exit\n"));
	...
	* puiErrorCode = uiError;
    _DebugPrint(( DebugLevelVerbose, "CWDMTVAudio:CWDMTVAudio() Error = %x\n", uiError));
}
