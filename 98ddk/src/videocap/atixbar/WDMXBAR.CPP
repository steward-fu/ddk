...
NTSTATUS CWDMAVXBar::AdapterCompleteInitialization( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:AdapterCompleteInitialization() ntStatus=%x\n",
		ntStatus));
...
BOOL CWDMAVXBar::AdapterUnInitialize( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:AdapterUnInitialize()\n"));
...
BOOL CWDMAVXBar::AdapterGetStreamInfo( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:AdapterGetStreamInfo()\n"));
...
BOOL CWDMAVXBar::AdapterQueryUnload( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:AdapterQueryUnload()\n"));
 ...
 * Author	: IKLEBANOV
 *^^*/
PVOID CWDMAVXBar::operator new( UINT size_t,  PVOID pAllocation)
{

	if( size_t != sizeof( CWDMAVXBar))
	{
		_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar: operator new() fails\n"));
		return( NULL);
	}
	else
		return( pAllocation);
...
CWDMAVXBar::~CWDMAVXBar()
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:~CWDMAVXBar() m_pXBarPinsInfo = %x\n", m_pXBarInputPinsInfo));
	...
	:m_CATIConfiguration( pDeviceObject, pCScript, puiErrorCode)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:CWDMAVXBar() enter\n"));
	...
	ENSURE
	{
		...
		for( nIndex = 0; nIndex < m_nNumberOfVideoOutputs; nIndex ++)
		{
			...
			m_pXBarOutputPinsInfo[nIndex].nRelatedPinNumber = 
                    m_nNumberOfVideoOutputs + m_nNumberOfAudioOutputs - 1;
...
*/
			...
			// all the pins here are outputs
			m_pXBarPinsDirectionInfo[nPinIndex] = TRUE;
            // output audio pins are only related to themselves
            m_pXBarOutputPinsInfo[nIndex + m_nNumberOfVideoInputs].nRelatedPinNumber = 
                    nIndex + m_nNumberOfVideoInputs;
		 ...
		 * puiErrorCode = WDMMINI_NOERROR;
		 _DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:CWDMAVXBar() exit\n"));
	...
	* puiErrorCode = uiError;
    _DebugPrint(( DebugLevelVerbose, "CWDMAVXBar:CWDMAVXBar() Error = %x\n", uiError));
}
			...
			if( nDeviceState != m_ulPowerState)
			{
				...
				if(( nDeviceState == PowerDeviceD0) && ( m_ulPowerState == PowerDeviceD3))
				{
					for( nIndex = 0; nIndex < m_nNumberOfAudioOutputs; nIndex ++)
					{
						...
						nInputPin = m_pXBarOutputPinsInfo[nIndex].nConnectedToPin;
