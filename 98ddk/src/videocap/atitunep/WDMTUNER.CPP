...
#pragma message ("\n!!! PAY ATTENTION: Tuner PinMedium is compiled without TVAudio support !!!\n")
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:AdapterCompleteInitialization() ntStatus=%x\n",
		ntStatus));
...
BOOL CATIWDMTuner::AdapterUnInitialize( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:AdapterUnInitialize()\n"));
...
BOOL CATIWDMTuner::AdapterGetStreamInfo( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:AdapterGetStreamInfo()\n"));
...
BOOL CATIWDMTuner::AdapterQueryUnload( PHW_STREAM_REQUEST_BLOCK pSrb)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:AdapterQueryUnload()\n"));
 ...
 * Author	: IKLEBANOV
 *^^*/
PVOID CATIWDMTuner::operator new( UINT size_t, PVOID pAllocation)
{

	if( size_t != sizeof( CATIWDMTuner))
	{
		_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner: operator new() fails\n"));
		return( NULL);
	}
	else
		return( pAllocation);
...
CATIWDMTuner::~CATIWDMTuner()
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:~CATIWDMTuner()\n"));
	...
	:m_CATIConfiguration( pDeviceObject, pCScript, puiErrorCode)
{
	...
	_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:CATIWDMTuner() enter\n"));
	...
	ENSURE
	{
		...
		* puiErrorCode = WDMMINI_NOERROR;
		...
		_DebugPrint(( DebugLevelVerbose, "CATIWDMTuner:CATIWDMTuner() exit\n"));
	...
	* puiErrorCode = uiError;
    _DebugPrint(( DebugLevelError, "CATIWDMTuner:CATIWDMTuner() Error = %x\n", uiError));
}
...
BOOL CATIWDMTuner::SetTunerWDMCapabilities( UINT uiTunerId)
{
	...
	switch( uiTunerId)
	{
		...
		case 0x06:		// NTSC North America + NTSC Japan
			m_ulNumberOfStandards = 1;
			m_wdmTunerCaps.ulStandardsSupported = KS_AnalogVideo_NTSC_M;
			m_wdmTunerCaps.ulNumberOfInputs = 1;
			m_wdmTunerCaps.ulMinFrequency =  55250000L;
		...
		case 0x08:		// NTSC North America NEC FM Tuner
			...
			m_wdmTunerCaps.ulMaxFrequency = 801250000L;
			m_ulIntermediateFrequency = 45750000L; 
			m_ulSupportedModes = KSPROPERTY_TUNER_MODE_TV | KSPROPERTY_TUNER_MODE_FM_RADIO;
			m_ulNumberOfPins = 2;
			break;
