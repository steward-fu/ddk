...
/*
 * New callbacks for post-DX3 Direct3D runtimes.
 *
 * This is queried out of the driver with GetDriverInfo and
 * GUID_D3DCallbacks2.
 *
 */

// This is a mirror of D3DHAL_CALLBACKS2 with no
// dependencies on Direct3D headers.

typedef struct _D3DHAL_CALLBACKS2_MIRROR
{
...
} D3DHAL_CALLBACKS2_MIRROR, *LPD3DHAL_CALLBACKS2_MIRROR;

#define D3DCALLBACKS2SIZE       sizeof(D3DHAL_CALLBACKS2_MIRROR)

typedef struct _D3DHAL_D3DEXTENDEDCAPS_MIRROR
{
    DWORD                       dwSize;                 // size of struct
...
} D3DHAL_D3DEXTENDEDCAPS_MIRROR, *LPD3DHAL_D3DEXTENDEDCAPS_MIRROR;

#define D3DEXTENDEDCAPSSIZE       sizeof(D3DHAL_D3DEXTENDEDCAPS_MIRROR)
...
typedef struct _DDRAWI_DDRAWPALETTE_GBL
{
    ...
    union
    {
        DWORD			dwReserved1;	// reserved for use by display driver which created this object
...
typedef struct _DDRAWI_DDRAWSURFACE_GBL_MORE
{
    ...
    DWORD       dwSavedDCContext;               // PRIVATE: For use by DDSurface::GetDC
    FLATPTR     fpAliasedVidMem;                // PRIVATE: Alias for original fpVidMem
    DWORD       dwDriverReserved;               // Reserved for driver use (both system and video memory surfaces)
    DWORD	dwHELReserved;			// PRIVATE: For HEL use only
    DWORD	cPageUnlocks;			// Incremented whenever a surface is PageUnlocked
    DWORD	hKernelSurface;			// Kernel handle for this surface
...
typedef struct _DDRAWI_DDVIDEOPORT_LCL
{
    ...
    DWORD 			dwReserved1;    // Reserved for display driver
    DWORD 			dwReserved2;    // Reserved for display driver
    DWORD                       hDDVideoPort;   // NT Kernel-mode handle
...
typedef struct _DDHAL_GETDRIVERINFODATA {
    ...
    DWORD	dwContext;	// Context Information
...
typedef struct _DDHAL_UPDATENONLOCALHEAPDATA
{
    ...
    DWORD                      dwReserved;         // PRIVATE: reserved for future use
    HRESULT                    ddRVal;             // return value
...
typedef struct _DDHAL_SYNCSURFACEDATA
{
    ...
    DWORD	dwDriverReserved1;	// Reserved for the HAL
    DWORD	dwDriverReserved2;	// Reserved for the HAL
    DWORD	dwDriverReserved3;	// Reserved for the HAL
    HRESULT	ddRVal;
